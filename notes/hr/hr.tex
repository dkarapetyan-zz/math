...
%
We consider the  initial value problem (IVP) for
...
In Yin \cite{Yin:2003ii} and Zhou 
\cite{Zhou:2005si}, a proof of local well-posedness in Sobolev 
...
Kato's semi-group theory \cite{Kato:1975dn}. 
For details on how this is done for CH on the line, see Rodriguez-Blanco 
\cite{Rodriguez-Blanco:2001co}. Blow-up criteria 
is also investigated in \cite{Yin:2003ii} and 
\cite{Zhou:2005si}, as well as by Constantin and Strauss 
\cite{Constantin:2000du}. 
...
\cite{Benjamin:1972xm}, 
\cite{Benjamin:1972ny}, and 
\cite{Constantin:2000du}).
For more general $\gamma$, the existence of global 
solutions to HR on the line with constant $H^1$ energy
was proved recently by Mustafa \cite{Mustafa:2007ou}
using the approach developed by Bressan and 
Constantin in \cite{Bressan:2007bq}. Using a vanishing 
viscosity argument, Coclite, 
Holden, and Karlsen \cite{Coclite:2005hd}
...
\cite{Bendahmane:2006oj} extended this result to traveling 
...
equation, see Holden and Raynaud \cite{Holden:2007hp}
and \cite{Yin:2003ii}. 
...
\cite{Lenells:2006mo}). Orbital stability of solitary wave 
solutions was proved in \cite{Constantin:2000du}.
Solitary shock wave formation was 
analyzed in Dai and Huo \cite{Dai:2000vo} using traveling 
...
Shiqiang Dai, and Huo \cite{Dai:2000cd} using a reductive 
...
\cite{Olson:2006rc} that the data-to-solution map
...
equation in 
$H^s(\rr)$ for $s>3/2$ was proven in \cite{Himonas:2009ck} 
...
\cite{Himonas:2007qf}.
Recently in \cite{Himonas:2010vp} non-uniform 
...
been studied in  \cite{Himonas:2007qf},
\cite{Himonas:2001db}, and 
\cite{Himonas:2005qg}, and for the Euler equations in 
\cite{Himonas:2010eu}. Continuity of this map for  
the Benjamin-Ono equation was studied in Koch and Tzvetkov 
\cite{Koch:2005pv}. For related ill-posedness results, we 
refer the reader to Kenig, Ponce, and  Vega 
\cite{Kenig:2001so}, Christ, Colliander, and Tao 
\cite{Christ:2003ez}, and the references 
therein.
...
Bona and Tzvetkov \cite{Bona:2009ww} have recently proved  that this equation  
...
mirrors  that in Himonas and Kenig \cite{Himonas:2009ck} and 
Himonas, Kenig, and Misio{\l}ek \cite{Himonas:2010vp}.
...
\cite{Li:2000nk} using a regularization method, and in
\cite{Rodriguez-Blanco:2001co} using 
Kato's semi-group method \cite{Kato:1975dn}. As mentioned 
above, proofs of 
existence, uniqueness, and continuous dependence for  HR
have been outlined in \cite{Yin:2003ii} and 
\cite{Zhou:2005si} for the line and circle, 
...
data-to-solution map for the HR IVP, expanding upon Theorem~\ref{thm:hr-non-unif-dependence}. More precisely, following \cite{Chen:2011aa} we show
...
This document is structured as follows. We first prove 
...
as it has been applied for the case $\gamma=1$ in \cite{Himonas:2009ck}.
...
coinciding with the approximate solutions at time zero such 
that for small time the size of the difference of solutions and approximate
solutions decays as $\lambda \to \infty$.
...
Following \cite{Himonas:2009ck}, our approximate solutions
...
proof can be found in \cite{Himonas:2009ck}: 
%
%
\begin{lemma}
\label{applea}
...
%
%
Next we prove the decay of the error as $\lambda \to \infty$:
%
%
\begin{proposition}
Let $1<\delta<2$. Then for $s > 1$ and $\omega$ in a bounded set
%
%
\begin{equation}
\label{E-est}
...
\end{lemma}
%
An analogous result can be found in \cite{Himonas:2009ck}, and is a simple consequence of Theorem~\ref{thm:HR_existence_continuous_dependence} and the profile of our initial data. Applying the inequality 
%
\begin{equation}
\begin{split}
...
%
%
For $E_{2}$, we apply \eqref{applec} to obtain
\begin{equation}
	\begin{split}
		\|E_2\|_{H^1(\rr)} 
...
%
%
For $E_{3}$ we see that \eqref{applec} implies
\begin{equation}
	\begin{split}
...
\end{equation}
%
For $E_{4}$ we apply \eqref{applec} to obtain
\begin{equation}
	\begin{split}
...
%
%
For $E_{5}$ we apply \eqref{apple27} to obtain
\begin{equation}
	\begin{split}
		\|E_5\|_{H^1(\rr)}
...
%
%
For $E_{6}$ we apply \eqref{apple27} to obtain
\begin{equation}
	\begin{split}
		\|E_6\|_{H^1(\rr)} 
...
%
%
For $E_{7}$ we apply \eqref{apple27} to obtain
\begin{equation}
	\begin{split}
		\|E_7\|_{H^1(\rr)} 
...
%
%
For $E_{8}$ we apply \eqref{apple27} to obtain 
\begin{equation}
	\begin{split}
		\|E_8\|_{H^1(\rr)}
...
%
%
We will now prove that the $H^1(\rr)$ norm of the difference decays as $\lambda \to \infty$: 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%
...
%
Let $v = u^{\omega,\lambda} - u_{\omega,\lambda}$.
...
%
%
We now estimate the three integrals on the right-hand side of 
...
%
%
A straightforward calculation of derivatives yields
%
%
\begin{equation*}
\begin{split}
...
%
%
\subsection{Non-Uniform Dependence for \texorpdfstring{$s>3/2$}{s > 3/2}}
...
\subsubsection{Behavior at time \texorpdfstring{$t=0$}{t=0}}  We have
%
%
%
%
...
\subsubsection{Behavior at time \texorpdfstring{$t>0$}{t>0}}  Using the reverse triangle inequality, we 
have
%
%
%
...
%
%
Letting $\lambda \to \infty$, we see that Lemma~\ref{applea}, Lemma~\ref{appleb}, and \eqref{apple90} give
%
%
%
%
...
\label{sec:3}
%
Here we follow the proof in \cite{Himonas:2010vp}. 
We will show that there there exist two sequences of solutions 
$u_n(t)$
...
satisfying the above properties.
Then, we will construct two sequences of actual solutions 
coinciding with the approximate solutions at time zero such that 
for small time the size of the difference of solutions and approximate solutions decays as $n \to \infty$.
...
infinite lifespan in $H^s(\ci)$ for $s  \ge 0$. Furthermore, we 
have 
%
%
\begin{equation}
...
used for the CH equation in \cite{Himonas:2010vp}.
%
%
...
%
%
Next we will prove that the error decays as $n \to \infty$:
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%
...
$\sigma \le 1$ and $\omega$ in a bounded set.
Then 
%
%
\begin{equation}
...
%
%
We will now prove that the size of the difference decays as $n \to \infty$: 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%
...
of \eqref{X}.
For the first integral, we applying Cauchy-Schwartz and obtain
%
%
\begin{equation}
\begin{split}
...
%
%
For the second integral, we rewrite
%
%
\begin{equation}
\begin{split}
...
taken from \cite{Himonas:2010vp}:
%
\begin{lemma}
\label{cor1}
...
For the third integral, we use Cauchy-Schwartz and recall that
$\sigma = 1/2 + \ee$ to obtain
%
%
\begin{equation}
\begin{split}
...
%
%
For the fourth integral, we will need the following result.
%
\begin{lemma}
...
n}$ and the associated approximate solution $u^{\pm 1, n}$ decays as $n \to \infty$.
We assume
...
\label{existence}
Here we will prove the existence of a solution to the HR IVP
and inequalities
...
theorem, taken from Dieudonn\`e \cite{Dieudonne:1969uc}: 
%
\begin{theorem}
\label{ode_theorem}
...
deal with this problem we will replace the HR IVP by  
\begin{gather}
\label{hr-moli}
\p_t  u_\ee =
...
%
%
\subsubsection{Energy Estimate for \texorpdfstring{$u_\ee$}{the Galerkin
approximator.}}
%
...
hyperbolic systems in Taylor \cite{Taylor:1991eg}. In what follows we will suppress the
...
%   
%
\subsubsection{Lifespan Estimate for \texorpdfstring{$u_\ee$}{the Galerkin
Approximator}} To derive an explicit formula for
...
%   
%
\subsubsection{Uniform Regularity of the set of
\texorpdfstring{$u_{\ee}$}{Galerkin Approximators}} If we choose  $T=\frac12 T_0$, that is
%
\begin{equation} 
\label{T-def}
T
...
the IVP for the mollified HR equation 
%
\begin{gather} 
\label{hr-moli-2}
\partial_t  u_\ee 
...
Using \eqref{hr-moli-2}, for any $t\in [-T, T]$ we have
%
\begin{equation*}
\begin{split}
...
whose limit $u$ solves the hyperelastic rod IVP\@. Let $I= [-T, T]$.
...
\end{equation}
%
In order to show that  $u$ solves the HR IVP we need to 
...
%%%%%%%%%%%%%%%%%%%%%%
%
Proceeding, we observe that Lemma~\ref{interpolation-lem} gives 
%
\begin{equation}
\label{equic-1}
...
\begin{enumerate}[(i)]
  ...
  \item{}  For each $x \in M$ that the set $S(x) = \{f(x)\}$  is  precompact in $X$.
\end{enumerate} 
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Verifying that the Limit Solves the HR Equation} 
...
to the HR IVP\@. In fact, $u \in L^{\infty}(I, H^{s}(\ci)) \cap \text{Lip}(I,
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Proof that the limit is in \texorpdfstring{$C(I, H^s(\ci))$}{C(I,
Hs(T))}} 
...
\end{split}
\end{equation}
To estimate the first integral, we rewrite
\begin{equation}
\begin{split}
  & -\frac{\gamma}{2} \int_{\ci} D^\sigma \p_x [v(u+w)] \cdot
D^\sigma v \ dx
...
\end{split}
\end{equation}
For the second integral, we apply Cauchy-Schwartz to obtain
\begin{equation*}
\begin{split}
|\frac{3-\gamma}{2} \int_{\ci}  D^{\sigma -2}
\p_x[v(u+w)] \cdot
...
\end{split}
\end{equation*}
We conclude that solutions to the HR IVP with initial data in
...
Let $\left\{ u_{0, n} \right\} \subset H^s(\ci)$ be a uniformly bounded
...
%
%
For the first integral, we can rewrite
%
%
\begin{equation}
\begin{split}
...
%
%
For the third integral, we apply Cauchy-Schwartz, the 
algebra property of Sobolev spaces, and the Sobolev Imbedding Theorem to obtain 
%
%
\begin{equation}
\begin{split}
...
%
%
For the fourth integral, we apply Cauchy-Schwartz, the 
algebra property of Sobolev spaces, and the Sobolev Imbedding Theorem to 
obtain
%
%
\begin{equation*}
...
%
%
We now aim to prove decay as $\ee \to 0$ for the $\ee^{-1}
...
\begin{equation*}
	\begin{split}
		...
		\right) \frac{dy}{dt} \lesssim \sqrt{1- o(1)} \sim 1.
	\end{split}
\end{equation*}
%
%
...
modifications. First, we must choose a different mollifier $J_\ee$ for the initial data in the
proof of continuous dependence. 
Secondly, in the proof of existence, we will have difficulties in arranging
that the solutions $\{u_\ee\}$ to the mollified HR IVP converge in $C(I,
H^{s- \sigma}(\rr))$, $0 < \sigma < 1$ to a candidate solution $u$ of the HR
IVP\@. We will get around this by considering the family $\left\{ \varphi
...
\end{gather}
%
where $J_{\ee}$ is defined as follows: Pick a
function $j(x) \in \mathcal{S}(\rr)$ and let 
\begin{equation*}
\begin{split}
...
L^\infty(I, H^{s-1}(\rr)). 
\end{align}
In order to show that $u$ solves the HR IVP, it would
...
IVP\@. Our focus will be on proving \eqref{hhstrong-conv}; since the proof of
...
there exists a sub-family $\{\varphi P[(u_{\ee_n}(t))^k]\}$
...
%
%
Proceeding, we observe that since $\varphi \in \mathcal{S}(\rr)$, the map $u \mapsto \vp u$
...
\end{equation*}
%
which shows that  the family  $\{\varphi P [(u_\ee)^k]\}$ is
...
%
%		
Furthermore, observe that by the algebra property of Sobolev
Spaces, and \eqref{the-map}-\eqref{op-norm-product}, we have
\begin{equation}
\begin{split}
\label{hhcompact-1}
...
\end{split}
\end{equation*}
continuously. Since $H^{s}$ is a Hilbert space, it follows that the adjoint operator $(T_\varphi P)^{\star}$
exists and
\begin{equation*}
...
the family $\{ \vp P [(\p_x u_{\ee})^k] \}$), one can also show
\begin{equation}
\begin{split}
...
\end{proposition}
\subsubsection{Verifying that the Limit Solves the HR Equation} 
We recall the mollified HR IVP 
\begin{gather}
...
existence of a solution to the HR IVP
\begin{lemma}
\label{hhlem:cc}
...
\subsubsection{Proof that the limit is in \texorpdfstring{$C(I, H^s(\rr))$}{C(I,
	Hs(R)}} The proof is analogous to that in the periodic case.
...
the periodic case, with one important caveat: we must choose an appropriate mollifier $J_{\ee}$ for the initial data. Pick a
...
\end{split}
\end{equation*}
Given this construction, the proofs of Lemma~\ref{lem5r} and Lemma~\ref{lem4r} for the non-periodic case are 
analogous to those in the periodic case.

Hence, how we
construct the mollifier $J_\ee$ plays a critical role in the proofs of
well-posedness for the HR IVP in both the periodic and non-periodic cases. 
%
