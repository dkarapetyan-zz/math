...
We follow \cite{Gorsky:2005qd}, and differentiate
...
\begin{gather*}
  ...
  \quad B_k = B_{k}(u,u) =
	\lambda \sum_{j=0}^{k} \sum_{i=0}^{j} \binom{k}{j}\binom{j}{i} u_{k-j} u_{j-i}
  \bar{u_i}
...
and Tao \cite{Colliander:2003hp}, and are a natural extension of the Bourgain spaces
\cite{Bourgain:1993ju}, \cite{Bourgain:1993ju}. Our main
...
\cite{Kato:2000uu}. Our strategy is to show the existence of solutions $\left\{
...
%
%
\subsection{Estimate for \ref{pmain-int-expression-1}.}
...
%
%
\subsection{Estimate for \ref{pmain-int-expression-2}.}
...
\begin{equation}
  ...
  \begin{split}
    ...
    \\
    & \lesssim 
		\sum_{j=0}^{k} \sum_{i=0}^{j} \binom{k}{j} \binom{j}{i}
...
\label{pprop:trilinear-est}
  ...
  \begin{equation}
    ...
    \right)^{1/2}
    \\
		& \lesssim \sum_{j=0}^{k} \sum_{i=0}^{j} \binom{k}{j} \binom {j}{i} 
		\|f_{k-j}\|_{X^s} \| g_{j-i}\|_{X^s}
    \| h_i \|_{X^s}
  \end{split}
  \end{equation}
  where $$B_k(f,g,h)(x,t) = \sum_{j=0}^{k} \sum_{i=0}^{j} \binom{k}{j} \binom {j}{i} 
 f_{k-j} g_{j-i} \bar{h_i}.$$
%
%
%
%
...
%
%
\subsection{Estimate for \ref{pmain-int-expression-3}.}
...
%
%
\subsection{Estimate for \ref{pmain-int-expression-4}.}
Note that
%
%
\begin{equation}
...
\begin{equation*}
\begin{split}
  & \sum_{k=0}^{\infty} \sum_{j=0}^{k}  \sum_{j=0}^{i} \binom{k}{j} \binom {j}{i} 
 \|u_{k-j}\|_{Y^s}
...
\begin{equation}
\begin{split}
  & \sum_{k=0}^{\infty} \frac{C_0^k}{k!} \sum_{j=0}^{k}  \sum_{j=0}^{i} \binom{k}{j} \binom {j}{i} 
 \|u_{k-j}\|_{Y^s}
\| u_{j-1}\|_{Y^s} \| u_{i} \|_{Y^s} 
\\
...
\begin{equation*}
  \begin{split}
    & \|L_k(f, g, h)\|_{Y^s} \le c_\psi \sum_{j=0}^{k} \sum_{i=0}^{j} \binom{k}{j} \binom {j}{i} 
 \|f_{k-j}\|_{Y^s} \| g_{j-i}\|_{Y^s} \| h_i \|_{Y^s}.
\end{split}
\end{equation*}
%
which implies
...
\begin{equation}
  ...
  \begin{split}
    ...
    \sum_{i=0}^{j}\frac{C_{0}^{k}}{k!} \binom{k}{j} \binom {j}{i} 
    \|u_{k-j} -v_{k-j} \|_{Y^s}
    \\
...
%
\section{\texorpdfstring{$G^2$}{G2}  regularity in time} 
\label{pg2-rreg-3}
%
%
...
Here we would like to find an analog to the construction
 ...
 (see  Gorsky-Himonas \cite{Gorsky:2005qd} and Byers-Himonas
 \cite{Byers:2006so})
 for NLS. 
