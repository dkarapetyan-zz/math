...
\begin{theorem}[Picard-Lindel\"{o}f]
	...
	%
	\end{theorem}
To prove \cref{ode-thm}, we will first define
...
\cite{Dieudonne:1969uc}, Yosida \cite{Yosida:1995ub},
Folland \cite{Folland:1999xi}, Jost
\cite{Jost:2005ra}, Rudin \cite{Rudin:1976ao}, and Knapp
\cite{Knapp:2005gd}-\cite{Knapp:2005gz}.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
\section{Proof of Metric Space ODE Theorem}
  ...
  \eqref{ode-thm-eq}-\eqref{ode-thm-init-data} is equivalent to the
	...
	Recalling \cref{lem:fixed-point}, we see that to complete the proof it will be enough to show that $T$ is a contraction on
...
\end{framed}
We now prove some useful corollaries on differentiating bilinear forms,
which will also allow us to illustrate Banach differentiation in action.
\begin{corollary}
		 \label{cor:dif_bilin}
	 Let
	 \begin{equation*}
	 \begin{split}
		 & f: \rr^{n} \times \rr^{n} \to \rr
		 \\
		 & f(z) = \langle x, y \rangle, \quad z=(x,y).
	 \end{split}
	 \end{equation*}
	Then 
	\begin{equation*}
	\begin{split}
	Df(z) = \langle x, \cdot \rangle  + \langle \cdot, y \rangle 
	\end{split}
	\end{equation*}
	 \end{corollary}
\begin{proof}
	...
	\\
	& = \langle x+h, y+h \rangle - \langle x,y \rangle 
	\\
	& = \langle x+h, y \rangle  + \langle x+h, h \rangle  - \langle x,y \rangle 
	\\
	& = \langle x,y \rangle  + \langle h,y \rangle  + \langle x, h \rangle  +
	\langle h,h \rangle - \langle x,y \rangle 
	\\
	& = \langle h,y \rangle  + \langle x,h \rangle + \text{O}(h^{2})
	\\
	& = L(z)(h) + \text{O}(h^{2}), \quad L(z) = \langle x, \cdot \rangle  + \langle \cdot, y \rangle
	\end{split}
	\end{equation*}
\end{proof}
	 \begin{corollary}
	...
	\begin{equation*}
	\begin{split}
	Df(z) = 2 \langle \cdot, x \rangle
	\end{split}
	\end{equation*}
	 \end{corollary}
\begin{proof}
	Repeat the proof for \cref{cor:dif_bilin}, with $x \doteq y$.
\end{proof}
...
reformulation of \cref{def:diff}. 
\begin{definition}
	\label{def:diff-simp}
...
%
%
Following Taylor \cite{Taylor:2011zy}, let $X$ be a Banach space, $U \subset X$ an open subset, and $F: U \to X$. Consider the non-autonomous system
%
%
\begin{gather}
  \label{aa-sm}
...
\begin{proof}
  We divide our work into cases.
	\subsection*{Case \texorpdfstring{$k = 0$}{k=0}.} The ivp \eqref{ode-eq-sm}-\eqref{ode-init-data-sm} is equivalent to 
  %
  %
  \begin{equation*}
  \begin{split}
...
%
concluding the proof.
\subsection*{Case \texorpdfstring{$k=1$}{k=1}} 
  \label{ssec:case-k}
 % 
 %
...
%
%
\section{Time Regularity for Solutions to ODEs with \texorpdfstring{$C^{k}$} Data} 
\label{sec:dep-param}
%
%
Following Taylor \cite{Taylor:2011zy}, let $X \subset \rr$ be an open subset,
...
\cite{Bona:2009ww}, we note that the BBM is also an ODE on
...
%
\begin{proof}
    We diverge slightly from the proof in \cite{Bona:2009ww}. Observe that
        \begin{equation*}
        \begin{split}
