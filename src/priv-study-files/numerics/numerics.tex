...
\end{theorem}
\begin{proof}
It is is a straightforward computation of $ \langle u_i, u_j \rangle $.
\end{proof}
...
By Lemma \ref{lem:hermite}, for each of $n$ linearly independent eigenvectors $u_i$ of $A^{\star}A$, for any 
positive associate eigenvalues $\sigma_i^2$. Furthermore,
\begin{equation}
\label{u-trans}
...
Then we call $A^{+} = Q^{\star}D^{+} P^{\star}$ it's \emph{pseudoinverse}.
\end{definition}
