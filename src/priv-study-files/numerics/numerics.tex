...
independent. To see this, factor $\lambda_1^j$ from row $j$, for each $1 \le j
\le m$. Then we obtain a Vandermonde matrix, which is invertible, by
Lemma \ref{vander}. Hence, $A$
is invertible, and so completely determines the $a_i$, for $1 \le i \le m$. 
As a corollary of this argument, we obtain that the vectors $u^{(k)}$ are linearly
independent. A simple induction on $m$ completes the proof. 
\item
...
\end{align*}
We now make a guess as to the solution of this recurrence.
Let $|e_{n+1}| \sim |e_n|^{\alpha}$, where $\alpha$ is a strictly
positive parameter to be determined. Then substituting into the above relation,
we must have
\begin{align*}
...
\end{align*}
However, our guess $ | e_{n+1} |  \sim | e_n | ^ {\alpha} $ implies
$ | e_{n-1} | \sim |e_n| ^{1/\alpha}$. Substituting this above, we obtain
\begin{align*}
|e_n|^{\alpha-1} \lesssim |e_{n}|^{1/\alpha}
\end{align*}
Hence, we must have $\alpha -1 = 1/\alpha$, or $\alpha^2 - \alpha - 1 = 0$,
which gives $\alpha = (1 + \sqrt{5})/2 \approx 1.62$.
Hence, the secant method, on its surface, converges slower to
a root than Newton's method. However, observe that the computational
cost of two iterations of the secant method is equivalent to cost
of one iteration of Newton's method (we have one function evaluation
at each iteration for the secant method, versus two for Newton's method).
Hence, if function evaluations are expensive, and we restrict the number of
computations to at most $n$ function evaluations, for some $n$, then the secant method converges $(2 \times 1.62)/2 =
1.62$ times as fast as Newton's method. 
