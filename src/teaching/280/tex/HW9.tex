...
\noindent
[6 points]1. \\ Recall a $m \times n$ matrix $\mathbb{A}$ has $n$ singular values $\sigma_1, \dots, \sigma_n$ which are the 
...
\noindent
[2 points]2. \\ For a $m \times n$ matrix $\mathbb{A}$ show that $\mathbb{A}^*\mathbb{A}$ and $\mathbb{A}$ have the same nullspace.
