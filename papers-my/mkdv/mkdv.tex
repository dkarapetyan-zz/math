...
%
\begin{document}
\title{Well Posedness for the mKDV}
...
%
\begin{definition}
	We say that the mKDV ivp \eqref{mKDV-eq}-\eqref{mKDV-init-data} is
	...
	\\
	$u \in C([-T, T],
	X)$ satisfying \eqref{mKDV-eq} for all $t \in [-T, T]$. 
...
%
%
Then it is immediate that \eqref{mKDV-integral-form} is a weaker 
restatement of the Cauchy-problem \eqref{mKDV-eq}-\eqref{mKDV-init-data}, 
since by construction any classical solution of the mKDV 
ivp is a solution to \eqref{mKDV-integral-form}. 
\\
\\
%
%
We now derive an integral 
equation global in $t$ and equivalent to \eqref{mKDV-integral-form} for $t 
...
$\eqref{mKDV-integral-form}$ by $\psi(t)$, we obtain
%
%
\begin{equation}
	\begin{split}
...
solution to the mKDV ivp \eqref{mKDV-eq}-\eqref{mKDV-init-data}.
\begin{definition}
