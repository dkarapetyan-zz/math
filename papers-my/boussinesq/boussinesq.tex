...
\begin{split}
  ...
  \wh{f}(n) \frac{e^{in^{2}t} + e^{-in^{2}t}}{2} 
  & + \psi(t) \sum_{n \in \zz} e^{inx}
  \wh{g}(n)\frac{e^{in^{2}t} - e^{-in^{2}t}}{2 i n^{2}} 
\end{split}
\end{equation*}
%
%
and for $u = u(x,t)$, $v = v(x,t)$ formally define 
%
%
\begin{equation*}
\begin{split}
...
\begin{definition}
  ...
  is \emph{qualitatively well-posed} in the
  spaces $H^{s} \times H^{s-2}$, $X_{s,b}$ if for all $(u_{0}, u_{1})$, $u, v
  \in X_{s,b}$ we have the estimates
  \begin{enumerate}
    \item{$\|L(u_{0}, u_{1})\|_{X_{s,b}} \le C \|(u_{0}, u_{1}) \|_{H^{s} \times
      H^{s-2}}$}
...
$H^{s} \times H^{s-2}, X_{s,b}$. Therefore, an application of Proposition 1 in
\cite{Bejenaru-Tao-2006-Sharp-well-posedness-and-ill-posedness} yields the
following.
%
\begin{proposition}
  Let $(u_{0}, u_{1}) \mapsto u(x, t)$ be the solution map to the localized
  $B_{4}$ which maps a ball $B_{H^{s} \times H^{s-2}}$ in $H^{s} \times
  H^{s-2}$ to a ball $B_{X_{s,b}}$ in $X_{s,b}$. Suppose that the solution map
  $(u_{0}, u_{1}) \mapsto u(x,t)$ is continuous from $B_{H^{s'} \times
  H^{s'-2}}$ to $(B_{X_{s,b}}, \| \|_{C([-\delta, \delta], H^{s'})})$ (i.e.\ the
  ball $B_{X_{s,b}}$ equipped with the $C([-\delta, \delta], H^{s'})$ topology),
  where $s' < s$. Then
  for  $n=1,2$, the operator $A_{n} :
  B_{H^{s'}} \times B_{H^{s'-2}} \to X_{s',b}$ is continuous
  from $B_{H^{s'}} \times B_{H^{s'-2}}$
  to $(B_{X_{s,b}}, \|  \|_{C([-\delta, \delta], H^{s'})})$,
  where
  %
  %
  \begin{equation*}
  \begin{split}
    &A_{1}(f,g) \doteq L(f,g),
    \\
    & A_{2}(f,g) \doteq N(A_{1}(f,g), A_{1}(f,g)).
  \end{split}
  \end{equation*}
  %
  %
...
\cos(\xi_{1}^{2}t') \wh{f_{N}}(\xi - \xi_{1})\wh{f_{N}}(\xi_{1}) d
\xi_{1}  dt'
\|_{L^{2}_{\xi}}
\\
& \gtrsim \sup_{0 \le t \le \delta} \| \chi_{| \xi | \le 1}
 \int_{0}^{t} \int_{\xi_{1}} \sin[(t-t')] \cos[(1 -\xi_{1})^{2}t']
 \cos(\xi_{1}^{2}t') \wh{f_{N}}(1 - \xi_{1})\wh{f_{N}}(\xi_{1}) d \xi_{1} dt'
 \|_{L^{2}_{\xi}}.
\end{split}
\end{equation}
%
...
\begin{split}
  \wh{f_{N}}(\xi) \doteq r N^{-s - \ee} \chi_{|\xi - N|\le 1}, \quad \ee > 0
\end{split}
\end{equation}
%
and fix $s < -1/2$. Then
...
%
%
Hence, for $s < -1/2$, $f_{N} \in B_{H^{s}}(2r)$
and $f_{N} \to 0$ in $H^{s}$. Therefore,
$(f_{N}, g_{N}) \in B_{H^{s} \times H^{s-2}}(2r)$ and $(f_{N}, g_{N}) \to 0$ in
$H^{s} \times H^{s-2}$. Substituting
...
sufficiently small we have the following.
\begin{enumerate}[(i)]
  \item
    \label{qwp-it-1}
...
\item
  \label{qwp-it-2}
  If we define the
nonlinear maps $A_{n}: H^{s} \times H^{s-2} \to C(\rr, H^{s})$ for $n = 1, 2,
\dots$ by
%
%
\begin{equation*}
...
%
%
then for all $f \in B_{H^{s} \times H^{s-2}}(r)$ we have the absolutely convergent (in $C([-\delta, \delta], H^{s})$ power series expansion
%
%
\begin{equation}
...
%
\begin{proof}
  It is enough to prove \cref{qwp-it-2}; the proof of \cref{qwp-it-1} was
  presented in \cref{sec:periodic} and \cref{sec:non-periodic-case}. In fact,
  we proved the linear and nonlinear estimates 
  %
  %
  \begin{align}
    \label{ill-pos-lin}
...
\begin{equation}
  \label{induct-ineq}
  \begin{split}
    \| A_{n}(f) \|_{X_{s,b}} \le c_{\psi}^{n} 2^{n-1} \delta^{(n-1)(b-1/2)} \| f
    \|_{H^{s} \times H^{s-2}}^{n}
  \end{split}
  \end{equation}
  %
  via an induction. For the base case, we have
  %
  \begin{equation*}
  \begin{split}
    \| A_{1}(f) \|_{X_{s,b}}
  ...
  \end{equation*}
  %
  For the inductive step, we assume \cref{induct-ineq} holds. 
  ...
  \begin{split}
    ...
    2^{k-1}\delta^{(k-1)(b-1/2)}\| f \|_{H^{s} \times
    H^{s-2}}^{k} \right]
    \\
    & = \sum_{\substack{j, k \in \mathbb{N}\\ j + k = n+1}} c_{\psi}^{j+k +1}
    2^{j+k-2} \delta^{(j + k -1)(b-1/2)} \| f \|_{H^{s} \times
    H^{s-2}}^{j+k}
    \\
    & \le 2^{n} \delta^{n(b-1/2)}
    c_{\psi}^{n+1} \| f \|^{n+1}_{H^{s} \times H^{s-2}}.
  \end{split}
  \end{equation*}
  %
  %
  Hence, by induction, \cref{induct-ineq}
  is true. Next, recalling \cref{qwp-it-1}, we see that there exists $u$ such that
  $Tu = u$; that is
  %
  %
  \begin{equation*}
