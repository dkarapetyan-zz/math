...
%
%
\begin{proposition}[Theorem 1.1 in~\cite{Farah:2010nq}]
\label{prop:bilinear-est}
	%
	%
	If $s > -a$ for fixed $a \ge 1/2$, then 
	\begin{equation}
	  \| w_{fg} \|_{\dot{X}_{s,-a}}
...
\begin{equation}
	...
	\begin{split}
	  &  \sup_{\|a_{n}\|_{\dot{\ell}^{2}}=1}| \sum_{n \in \zzdot} \left (1 + |n| \right )^{s} a_n
		...
		\right)^{1/2} 
		\left ( \sum_{n \in \zzdot}\int_{\rr} \left (1 + |n| \right )^{2s} (1 + | | \lambda | - \gamma(n) |)^{\eta -1} \wh{w}(n, \lambda) 
		|^2\ d \lambda 
		\right)^{1/2}
		\\
...
\begin{split}
  ...
  \\
  &  \lesssim_{\ee} \sum_{n \in \zzdot} \frac{\chi_{A_{3,1}}\langle n - n_{1} \rangle ^{-2s-2a } \langle n_{1} \rangle ^{-2s-2a}}{\langle
    n\rangle ^{-2s}}
      \end{split}
\end{equation}
%
...
\end{split}
  \end{equation*}
  %
  and applying \cref{lem:calc}, \cref{lem:key-trick}, and the fact that 
  $0 < | n | < | n_{1} |$ in region $A_{3,1}$, we see that we can bound \eqref{region-4-1} by
  %
  %
  \begin{equation*}
  \begin{split}
    \sum_{n \in \zzdot} \frac{\langle n_{1}\rangle ^{\gamma(s)-4a +\ee}}{\langle n \rangle^{1 + \ee}} < \infty, \quad s \ge -a + \ee/4
  \end{split}
  \end{equation*}
  %
  for $\ee > 0$, uniformly in $n_{1}$.
  %
...
\begin{align*}
  ...
  A_{4,3}&=\{(n, n_1, \tau, \tau_1)\in A_4: |\tau - \tau_{1}-(n - n_{1})^2|\ge \frac{2}{3} |n_{1}||n - n_{1}| \}.
\end{align*} 
Hence, applying \cref{lem:calc} and \cref{lem:key-trick}, we obtain 
%
%
%
%
...
For region $B_{3,2}$, we apply a change of variable to obtain
\begin{equation}
\begin{split}
...
\end{equation}
%
and the rest of the argument is identical to that of region $A_{3,2}$.
