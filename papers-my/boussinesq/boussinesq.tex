...
\label{prop:bilin-est}
  Let $s \ge 0 $ and $u,v \in X_{s, -a}$. If $b > 1/2$, and $1/4 < a< 1/2$ 
...
\begin{definition}
  ...
  topology (i.e. $\|(f_0, f_1)\|_{X \times Y} = \|f_0\|_{X} + \|f_1\|_{Y}$).
...
Let $A \subset \rr^{2} \times \zz^{2}$, and $\chi_{A}(\tau, \tau_{1}, n,
n_{1})$ be its
...
%
%
Now consider the family $\{A_{j}\}_{1}^{n}, A_{j} \subset \rr^{2} \times
\zz^{2}$ with
$$\bigcup_{1}^{n} A_{j}= \rr^{2} \times
...
\end{split}
\end{equation}
for each $j \in \left\{ 1,\dots,n \right\}$. 
...
\end{align*} 
%
For pedagogical purposes, we seek to bound 
%
%
\begin{equation*}
  \begin{split}
...
\end{remark}
\end{framed}
Let's try something different, and see if we can improve our result.
...
\begin{split}
  ...
  \langle \tau_{1} + n_{1}^{2} \rangle
  ^{2a + 2b}} < \infty, \qquad s \ge 0.
\end{split}
\end{equation*}
%
%
Hence, the existence of the
``pathological'' term for the $B_{4}$ equation has lowered the optimal periodic result a
$1/4$ derivative. 
%
%
Appling \autoref{lem:calc}, we now bound 
\begin{equation*}
  \begin{split}
    & \frac{ \langle n
    \rangle ^{2s}}{\langle \tau - n^{2} \rangle ^{2a}}
...
$A_{4,1}$, we apply \autoref{lem:calc} and \eqref{smoothing-per-4-1}
to obtain
%
%
%
...
\begin{split}
  \frac{\langle n \rangle ^{2s} \chi_{A_{4,1}}}{\langle n_{1} \rangle ^{2s} \langle n -
...
n \rangle ^{-2s} \langle n_{1} \rangle
^{2a}} \le 1, \quad s \ge 0 \text{ or } s < 0, a \ge 2s
\end{equation}
%
%
...
%
%
where the last line follows from \eqref{growth-term-control-per}. Applying
\autoref{lem:calc}, \eqref{smoothing-per-4-2}, and \eqref{growth-term-control-per}
...
&  \frac{C}{\langle n_{1} \rangle ^{2s}
  ...
  \\
  & \lesssim \frac{1}{\langle n_{1} \rangle ^{2s}}
  \sum_{n \in \zz} \frac{\langle n \rangle ^{2s}}{\langle
  n - n_{1}\rangle ^{2s}\langle n_{1} \rangle ^{2a}}
  \times \frac{\chi_{A_{4,2}}}{\langle
  \tau_{1} - 2nn_{1} + n_{1}^{2} \rangle ^{2b}} 
  \\
  & \le \frac{1}{\langle n_{1} \rangle ^{2s}}
  \sum_{n \in \zz} \frac{\chi_{A_{4,2}}}{\langle
  \tau_{1} - 2nn_{1} + n_{1}^{2} \rangle ^{2b}},
  \quad s \ge 0 \text{ or } s < 0, a \ge 2s
  \\
  & \le 
  \sum_{n \in \zz} \frac{\chi_{A_{4,2}}}{\langle
  \tau_{1} - 2nn_{1} + n_{1}^{2} \rangle ^{2b}},
...
%
%
For pedagogical purposes, we seek to estimate 
%
%
\begin{equation}
...
\begin{framed}
  \begin{remark}
    Lets try something different, and see if we can improve our result.
    We seek to bound
\begin{equation}
\begin{split}
  & \frac{1}{\langle n_{1} \rangle ^{2s}
...
%
%
By \autoref{lem:calc}, this is bounded by
%
%
\begin{equation*}
\begin{split}
...
%
%
which in turn is bounded for $s \ge 0$. Hence, the existence of the
``pathological'' term for the $B_{4}$ equation has lowered the optimal periodic result a
$1/4$ derivative. 
\end{remark}
\end{framed}
...
%
We now 
partion $ B_{3}$ into four parts
\begin{align*}
B_{3,1}&=\{(n, n_1, \tau, \tau_1)\in B_3:
|\tau-\tau_1-(n-n_1)^2|\leq|\tau_1-n_1^2| \le |\tau-n^2|\},\\
B_{3,2}&=\{(n, n_1, \tau, \tau_1)\in B_3:
|\tau-\tau_1-(n-n_1)^2|\leq|\tau_1-n_1^2|, |\tau-n^2| \le |\tau_1-n_1^2|\},\\
B_{3,3}&=\{(n, n_1, \tau, \tau_1)\in B_3: |\tau_{1}-n_{1}^2|\leq|\tau -
\tau_{1}-(n - n_{1})^{2}| \le | \tau - n^{2} |\}, \\
B_{3,4}&=\{(n, n_1, \tau, \tau_1)\in B_3: |\tau_{1}-n_{1}^2|\leq|(\tau -
\tau_{1})-(n - n_{1})^{2}|,  | \tau - n^{2} | \le |  \tau - \tau_{1} -
(n - n_{1})^{2} |\},
\end{align*} 
...
\begin{equation*}
  \begin{split}
    \frac{ \langle \xi
    \rangle ^{2s}}{\langle \tau - \xi^{2} \rangle ^{2a}}
    \int_{\rr} \frac{\chi_{A_{2}}}{ \langle \xi_{1} \rangle ^{2s} \langle \xi-\xi_{1} \rangle ^{2s} 
    \langle \tau - \xi^{2} +2 \xi \xi_{1} \rangle^{2b} } d \xi_{1}.
  \end{split}
\end{equation*}
%
We now split $A_{2}$ into the sets
%
%
%
\begin{equation*}
...
& A_{2,1} = 
  \begin{Bmatrix}
    | \xi_{1}| \ge 1 \text{ and } | \xi - \xi_{1} | \ge 1,
    \\
    \langle \tau - \tau_{1} -(\xi -\xi_{1})^{2} \rangle \le \langle \tau_{1} + \xi_{1}^{2} \rangle  \le \langle \tau -
  \xi^{2} \rangle
\end{Bmatrix},
\\
& A_{2,2} = 
\begin{Bmatrix}
    ...
    \\
     \langle \tau_{1} + \xi_{1}^{2} \rangle  \le \langle \tau -
     \xi^{2} \rangle \text{ and } \langle \tau_{1} + \xi_{1}^{2}\rangle 
     \le \langle \tau - \tau_{1} -(\xi -\xi_{1})^{2} \rangle
\end{Bmatrix}.
\end{split}
\end{equation*}
%
...
%
%
Hence, applying and \eqref{growth-term} and \eqref{case-2-region-A2-key-est}, we obtain
%
%
%
%
...
\begin{split}
  ...
  & \lesssim 
  \int_{\rr} \int_{\rr} \frac{\chi_{A_{2,1}} \langle \xi_{1}
  \rangle^{\gamma(s) -2a}}{  
    \langle \tau_{1} + \xi_{1}^{2} \rangle^{2b} \langle  \tau - \tau_{1} -
    (\xi - \xi_{1})^{2} \rangle^{2b} }
    d \tau_1 d \xi_{1}
    \\
    & \le \int_{\rr} \int_{\rr} \frac{\chi_{A_{2,1}}}{  
    \langle \tau_{1} + \xi_{1}^{2} \rangle^{2b} \langle  \tau - \tau_{1} -
    (\xi - \xi_{1})^{2} \rangle^{2b} }
    d \tau_1 d \xi_{1}, \quad s \ge -\frac{a}{2}.
\end{split}
...
\end{split}
\end{equation*}
This concludes the proof of \autoref{prop:bilin-est-real}. \qquad \qedsymbol
%
%Similarly, 
%%
%%
