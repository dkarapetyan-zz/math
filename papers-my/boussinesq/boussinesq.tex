...
For establishing well-posedness for large data via cutoffs, it
...
as $\delta$ approaches $0$. Later, when handling the endpoint case $b=1/2$, $s=-1/2$, we shall introduce a scaling approach that extends small data well-posedness to large data well-posedness, bypasses the above difficulty entirely, and works not just for the endpoint case, but for $s > -1/2$ as well.
\end{remark}
\end{framed}
...
\begin{equation*}
	\begin{split}
	  & g_k(x,t) = t^k \psi(t) \sum_{n \in \zz} e^{i\left( xn + ta \gamma(n)
		\right)} h_k(n),
		\\
...
\end{split}
  ...
  \end{equation*}
  %
  uniformly in $(n, \tau)$. This follows from the following lemma,
...
\begin{equation*}
\begin{split}
\frac{1}{\langle \tau_{1} + n_{1}^{2} \rangle } \lesssim \frac{n_{1}^{2}}{\langle \tau_{1} - n_{1}^{2} \rangle^{2}}. 
\end{split}
\end{equation*}
%
%
...
\begin{equation}
\begin{split}
  & \frac{1}{M}\| \int_{\xi} \int_{\tau} \frac{\langle \xi \rangle ^{2s+2}}
...
%
%
where $\cup A_{j} = \rr^{4}$, for cases \labelcref{it-4-np} and \labelcref{it-6-np}. The analysis is analogous to that in the periodic case, due to the following non-periodic analogue to \cref{lem:sum-estimate}.
\begin{lemma}
