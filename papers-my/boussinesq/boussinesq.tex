...
that the computation of the second iterate yields a factor
of $N^{-1}$
...
\begin{theorem}
  ...
  (f_{N,1}(x), f_{N,2}(x)) = \left ( \frac{r N^{-\beta}}{2 \pi}\left[ e^{iNx} + e^{i(1-N)x} \right], 0
  \right ), \quad \beta = 1/4 + 2 \ee.$$ Then 
  %
  %
    \begin{enumerate}[(I)]
      \item{$ f_{N} \in B_{H^{-1/4 + \ee} \times H^{-5/2 + \ee}}(r) \ \text{with associated
    solutions}
    ...
    \\
  \item
    $ \|A_{2}[f_{N}]\|_{C\left( [-\delta, \delta, H^{s}] \right)}/ \| f_{N}
    \|_{H^{s}} \not \to 0$. 
\end{enumerate}
  %
  %
...
\begin{split}
  \wh{f_{N,1}}(n)
  & = r N^{-\beta}\int_{\ci} [e^{-ix(n - N)} + e^{-ix(n - (1-N))}]  dx  
  \\ 
  & = 
  \begin{cases}
    2 \pi r N^{-\beta},  \quad  & n = N \ \text{or} \ n = 1-N
    \\
...
\begin{split}
  \| f_{N,1} \|_{H^{s}}
  & = \frac{2 \pi r N^{-\beta}}{2 \pi} \left( \sum_{n \in \zz} (1 + | n |)^{2s} |
...
\begin{equation*}
  \begin{split}
    & (f_{N,1}, 0) \in B_{H^{-1/4 + \ee} \times H^{-5/2 + \ee}}(r)
    \end{split}
\end{equation*}
Note that since we have assumed well-posedness for the $B_{4}$ equation for $s
\ge -1/2$, and since $f_{N} \in B_{H^{s} \times H^{s-2}}(r)$, the associated
...
\begin{equation*}
\begin{split}
  & \| A_{2}(f_{N}) \|_{C([-1, 1], H^{s})} 
  \\
& \ge \| A_{2}(f_{N}) \|_{C([1/2, 1], H^{s})} 
\\
  & =  \| N[A_{1}(f_{N}), A_{1}(f_{N})] \|_{C([1/2, 1],
  H^{s})} 
  \\
  & = \sup_{1/2 \le t \le 1} \| \psi_{1}(t) (1 + | n |)^{s}
  \frac{1}{4 i \pi}
  \int_{1/2}^{t} \left( e^{in^{2}(t-t')} - e^{-in^{2}(t-t')} \right)
  \wh{[L(f_{N})]^{2}}(n, t') dt' \|_{\ell^{2}_{n}}
  \\
  & = \sup_{1/2 \le t \le 1} \| (1 + | n |)^{s} \frac{1}{4 i \pi} 
  \int_{1/2}^{t} \sum_{n_{1}}
  \\
  & \left( e^{in^{2}(t-t')} - e^{-in^{2}(t-t')} \right)
  \wh{L(f_{N})}(n - n_{1}, t')\wh{L(f_{N})}(n_{1}, t') dt'
  \|_{\ell^{2}_{n}}
  \\
  &= \sup_{1/2 \le t \le 1} \frac{1}{16 i \pi^{2}}\| (1 + | n |)^{s}
  \int_{1/2}^{t} \sum_{n_{1}} \left( e^{in^{2}(t-t')} - e^{-in^{2}(t-t')} \right)
  \\
...
\begin{equation}
  \label{pre-loc}
\begin{split}
& \sup_{1/2 \le t \le 1} \frac{1}{8 \pi^{2}}\| (1 + | n |)^{s}
\int_{1/2}^{t} \sum_{n_{1}} \sin[n^{2}(t-t')] \cos[(n - n_{1})^{2}t']
...
\begin{equation}
  \label{yut}
\begin{split}
  & \frac{2^{s}}{8\pi^{2}} \sup_{1/2 \le t \le 1} 
  | \int_{1/2}^{t} \sum_{n_{1}} \sin(t-t') \cos[(1 -n_{1})^{2}t']
...
\begin{equation}
  \label{rxx}
\begin{split}
  & \frac{2^{s}}{4\pi^{2}}  N^{-2 \beta} \sup_{1/2 \le t \le 1} 
 | \int_{1/2}^{t} \sin(t-t') [\cos(N^{2}t')] [\cos( (1 - N)^{2}t')] dt' |
 \\
 & \gtrsim N^{-2 \beta} | \int_{1/2}^{1} \sin(1-t') [\cos(N^{2}t')] [\cos( (1 -
 N)^{2}t')] dt' |.
\end{split}
\end{equation}
%
Let $N = \sqrt{2 \pi k}, k \in \mathbb{N}$. Noting that 
%
%
\begin{equation*}
\begin{split}
  (1 - N)^{2} = \left( \frac{N-1}{N} \right)^{2} N^{2}
\end{split}
\end{equation*}
%
%
and choosing $k$ sufficiently large such that $\left ( \frac{N-1}{N} \right
)^{2} > 0.99$, we bound \eqref{rxx} below by
%
%
%
\begin{equation*}
\begin{split}
\frac{1}{4}
& N^{-2 \beta} | \int_{1/2}^{1} \sin(1-t') dt' |
\\ & \simeq N^{-2 \beta}.
\end{split}
\end{equation*}
%
...
\begin{equation*}
\begin{split}
  \frac{\|A_{2}[f_{N}]\|_{C\left( [-\delta, \delta, H^{s}] \right)}}{\| f_{N}
  \|_{H^{s}}} 
  & \gtrsim \frac{N^{-2 \beta}}{N^{s-\beta}}
  \\
  & = N^{-s - \beta}
  \\
  & = N^{-s - 1/4 + \ee}
  \\
  & \ge 1, \quad s \le -1/4 - \ee
\end{split}
\end{equation*}
%
%
