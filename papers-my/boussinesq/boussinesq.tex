...
\begin{equation}
  \label{fh}
\begin{split}
  4  \left( \sum_{n \in \zzdot} \left (1 + |n| \right )^{2s}  \left ( \int_\rr
...
\begin{equation*}
	\begin{split}
	  \sup_{\|a_{n}\|_{\dot{\ell}^{2}}=1}| \sum_{n \in \zzdot} \left (1 + |n| \right )^{s}
		a_n \int_{\rr} \frac{|\wh{w}(n, \lambda)|}{1 
		+ | |\lambda| - \gamma(n) |} \ d \lambda |
	\end{split}
\end{equation*}
%
%%
...
\begin{equation}
	\label{n1m}
	\begin{split}
	  &  \sup_{\|a_{n}\|_{\dot{\ell}^{2}}=1}| \sum_{n \in \zzdot} \left (1 + |n| \right )^{s} a_n
		\int_{\rr}\frac{| \wh{w}(n, \lambda) |}{1 + | |\lambda| - \gamma(n) |} \ d \lambda |
		\\
		& \le \sup_{\|a_{n}\|_{\dot{\ell}^{2}}=1} \left( \sum_{n \in \zzdot} | a_{n} |^2\int_{\rr} \frac{1}{\left( 1 + | |\lambda| - \gamma(n) | \right)^{1 + \eta}} \ d \lambda  
		\right)^{1/2} 
		\left ( \sum_{n \in \zzdot}\int_{\rr} \left (1 + |n| \right )^{2s} (1 + | | \tau | - n^{2} |)^{\eta -1} \wh{w}(n, \lambda) 
		|^2\ d \lambda 
		\right)^{1/2}
		\\
  & \lesssim \|  w \|_{\dot{X}_{s, -(1- \eta)}}, \quad \eta > 0.
\end{split}
\end{equation}
%
%
...
\begin{equation*}
  ...
  c_{v}^{2} (n - n_1, \tau - \tau_{1} ) d \tau_1  \right)^{1/2} \|_{L^{2}(\zzdot \times
    ...
    \rangle ^{s} \langle \tau - n^{2} \rangle ^{-a}
		\\
...
\begin{split}
  ...
  \int_{\tau_{1}} \chi_{A} \phi(n, \tau)
    c_f(n_1, \tau_1)
    c_g(n - n_1, \tau - \tau_1 ) \langle | \tau | - n^{2} \rangle ^{-a}
		\\
    & \times \frac{\langle n \rangle ^{s}}{\langle n_{1} \rangle ^{s} \langle
...
\end{split}
  \end{equation*}
  %
  and applying \cref{lem:calc} and \cref{lem:key-trick},
  we see that for $s \ge -a + \ee$, we can bound \eqref{region-4-1} by
  %
  %
  \begin{equation*}
  \begin{split}
    \sum_{n \in \zzdot} \frac{\langle n_{1}\rangle ^{\gamma(s)-2 -\ee}}{\langle n \rangle^{1 + \ee}} < \infty, \quad s \ge -a + \ee
  \end{split}
  \end{equation*}
  %
  for $0 < 1 < \ee$, uniformly in $n_{1}$.
  %
...
\end{equation}
%
uniformly in $n_{1}$, $\tau_{1}$. The remainder of the argument is analogous to that of region $A_{3,1}$. The analysis of region $A_{3,3}$ is subsumed by that of $A_{3,2}$, thanks to the symmetry of the convolution.
  %
  %
  %
  %
...
%
\begin{align*}
B_{1} &= \{(n, n_1, \tau, \tau_1)\in B: n_{1}=n, n \neq 0\},\\
...
\begin{align*}
  B_{3,1}&=\{(n, n_1, \tau, \tau_1)\in B_3: |\tau-n^{2}|\ge \frac{2}{3} |n_{1}|| n - n_{1} |\},\\
  B_{3,2}&=\{(n, n_1, \tau, \tau_1)\in B_3: |\tau_{1}-n_{1}^2|\ge \frac{2}{3} |n_{1}||n - n_{1}| \}\\
  B_{3,3}&=\{(n, n_1, \tau, \tau_1)\in B_3: |\tau - \tau_{1}-(n - n_{1})^2|\ge \frac{2}{3} |n_{1}||n - n_{1}| \}.
\end{align*} 
%
%
The analysis for region $B_{3,1}$ is analogous to that for region $A_{3,1}$. 
For region $B_{3,2}$, we apply a change of variable, \cref{lem:calc} and \cref{lem:key-trick} to obtain
  \begin{equation}
\begin{split}
  &  \frac{1}{\langle n_{1} \rangle^{2s}} \sum_{n \in \zzdot} \int_{\tau} \frac{\langle n \rangle ^{2s}}{\langle n - n_{1}\rangle ^{2s}}  \times \frac{\chi_{B_{3,2}}}{\langle \tau - n^{2} \rangle^{2a} \langle
      ...
      & = 
      \frac{1}{\langle n_{1} \rangle^{2s}} \sum_{n \in \zzdot} \int_{\tau} \frac{\langle n \rangle ^{2s}}{\langle n - n_{1}\rangle ^{2s}}  \times \frac{\chi_{B_{3,2}}}{\langle \tau - n^{2} \rangle^{2a} \langle
      \tau_{1} - n_{1}^{2} \rangle^{1 \pm \ee}  \langle \tau + \tau_{1} - (n - n_{1})^{2} \rangle} d \tau
\\
&  \lesssim_{\ee} \sum_{n \in \zzdot} \frac{\langle n - n_{1} \rangle ^{-2s-1-\ee} \langle n_{1} \rangle ^{-2s-1 + \ee}}{\langle
    n\rangle ^{-2s}}
      \end{split}
\end{equation}
%
and the rest of the argument is identical to that of region $A_{3,1}$
%
Similarly, for region $B_{3,3}$ we have
%
\begin{equation}
\begin{split}
  &  \frac{1}{\langle n_{1} \rangle^{2s}} \sum_{n \in \zzdot} \int_{\tau} \frac{\langle n \rangle ^{2s}}{\langle n - n_{1}\rangle ^{2s}}  \times \frac{\chi_{A_{3,1}}}{\langle \tau - n^{2} \rangle \langle
      \tau_{1} - n_{1}^{2} \rangle  \langle \tau - \tau_{1} - (n - n_{1})^{2} \rangle} d \tau
  \\
  & = 
  \frac{1}{\langle n_{1} \rangle^{2s}} \sum_{n \in \zzdot} \int_{\tau} \frac{\langle n \rangle ^{2s}}{\langle n - n_{1}\rangle ^{2s}}  \times \frac{\chi_{A_{3,1}}}{\langle \tau - n^{2} \rangle \langle
  \tau_{1} - n_{1}^{2} \rangle  \langle \tau - \tau_{1} - (n - n_{1})^{2} \rangle^{1 \pm \ee}} d \tau
\\
&  \lesssim_{\ee} \sum_{n \in \zzdot} \frac{\langle n - n_{1} \rangle ^{-2s-1 + \ee} \langle n_{1} \rangle ^{-2s-1 + \ee}}{\langle
    n\rangle ^{-2s}}
      \end{split}
\end{equation}
%
and the rest of the argument is identical to that of $A_{3,1}$. 
