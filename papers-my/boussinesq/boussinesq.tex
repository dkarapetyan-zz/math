...
%
%
\subsection{Proof of \cref{prop:ill-posedness}}
...
\cref{main1-rel-term-0}-\cref{main1-rel-term-5}. Furthermore, $u =
\psi_{\delta} u$. 
\item
  \label{homogen}
...
(in $X_{s,b} \cap C([-\delta, \delta], H^{s})$) power series expansion
%
%
\begin{equation}
...
\begin{equation*}
\begin{split}
  \sum_{n' > n} c_{\psi}^{n'} 4^{n'-1} 
  \delta^{(n'-1)(b-1/2)}  \lambda^{n'-n} r^{n'}
  & = 2 c_{\psi} r \sum_{n' > n} c_{\psi}^{n'-1} 4^{n'-1} 
  \delta^{(n'-1)(b-1/2)}  \lambda^{n'-n} (2r)^{n'-1}
  \\
  & = 2 c_{\psi} r \sum_{n' > n} \lambda^{n'-n} (8 rc_{\psi}
  \delta^{(b-1/2)})^{n'-1}
  \\
  & = 2 c_{\psi} r \sum_{k =1}^{\infty} \lambda^{k} (8 rc_{\psi}
  \delta^{(b-1/2)})^{k +n -1}
  \\
  & = 2 c_{\psi} r (8 rc_{\psi}
  \delta^{(b-1/2)})^{n -1}
\sum_{k =1}^{\infty}  (8 \lambda rc_{\psi}
  \delta^{(b-1/2)})^{k},
  \\
  & \le \frac{2 c_{\psi} r (8 rc_{\psi}
  \delta^{(b-1/2)})^{n -1}}{1-8 \lambda r c_{\psi}
  \delta^{(b-1/2)}}, \qquad \lambda \le \frac{1}{16rc_{\psi} \delta^{(b-1/2)}}.
\end{split}
\end{equation*}
%
Recalling \cref{pre-lambda-small-app}, we see that for any $\lambda < 1/(16 r
c_{\psi} \delta^{b-1/2})$, we have
%
%
\begin{equation*}
\begin{split}
...
%
\subsubsection{Proof of \cref{lem:qwp-awp}} We split the proof into five parts.
...
\begin{proof}[Proof of \cref{qwp-it-1}]
  ...
  \end{align}
  %
  where we have the restrictions $u = \psi_{\delta} u$, $v = \psi_{\delta} v$.
...
\cref{induct-ineq}, this is bounded by
%
%
\begin{equation*}
\begin{split}
...
Next, recalling \cref{qwp-it-1}, we see that there exists $u$ such that $u =
\psi_{\delta} u$ and 
...
Using this as motivation, we will now estimate the error of $u_{k} \doteq
  ...
  \end{split}
\end{equation*}
Since $u = \psi_{\delta} u$, we have $A_{k}(f) = \psi_{\delta} A_{k}(f)$ for all
$k > 1$. Hence, applying \cref{ill-pos-bil} and \cref{induct-ineq}, we bound
above by
\begin{equation*}
  \begin{split}
