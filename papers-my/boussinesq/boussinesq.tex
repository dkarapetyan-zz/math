...
%
%
\section{Ill-Posedness for $s < -1$ on the Circle}
...
\\
& \lim_{N \to \infty} \gtrsim \frac{\sin(N^{-\alpha}) N^{-\alpha -1}}{N^{2s + 2\ee - 1}}
\\
...
\begin{split}
  ...
  - t'}) + e^{i[N^{-\alpha} - (N-1)^{2}t' -t']} - e^{-i[N^{-\alpha} - t' -
...
$-3s -3 \ee -2 \ge 4$, which gives $s \le -2 -\ee$. This is the problem term
which limits how good a ill-posedness result we can get (computations coming
shortly). Hence, considering the 
higher iterate $A_{2}$ has given us no better an ill-posedness result than
considering the $A_{1}$ iterate. 
\newpage
\begin{appendices}
