...
\begin{theorem}
  ...
  < 1/4$ in the periodic and non-periodic cases.
\label{thm:wp-2}
\end{theorem}
%
%
...
\begin{framed}
\begin{remark}
  I do not see why Farah takes initial data $\p_t (x, 0) = \psi_{x} \in
  H^{s-1}(\ci)$. I take initial data $\p_t (x, 0) = u_{1}(x) \in
  H^{s-2}(\ci)$ with no problems. The proof of well-posedness does not invoke
  the existence of non-existence of mean initial data in any way.
%
\end{remark}
\end{framed}
%
...
\begin{framed}
  \begin{remark}
    Using the $Y^{s}$ spaces of CCSTT, I believe we can sharpen the
    well-posedness result to $s \ge -1/4$.
    (Farah mentions this is currently an open problem). 
  \label{rem:improved-result}
  \end{remark}
\end{framed}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{gather}
  \label{sin-cos-op}
  \wh{R_{t}\vp}(n) = \wh{\vp}(n) \frac{e^{in^{2}t} + e^{-in^{2}t}}{2} , \quad 
  \wh{S_{t}\vp}(n) = \wh{\vp}(n) \frac{e^{in^{2}t} - e^{-in^{2}t}}{2i n^{2}}.
\end{gather}
%
...
= [-1, 1 ]$. Define $\psi_{T}(t) = \psi(t/T)$, $ 0 < T \le 1$.
Motivated by $\eqref{eqn:integral-form}$, we now consider the equation
%
%
\begin{equation}
  \begin{split}
...
\end{equation}
where $L=L_{u_0, u_1, \psi, T}$. Note that if $u$ is a solution of
\eqref{localized-int-eqn}, then $\widetilde{u} =u|_{\left[ -T, T \right]}$ is a
solution of \eqref{eqn:integral-form}. We now introduce the following spaces. 
%
%
\begin{definition}
...
\subsubsection{Estimate for $\psi_{T}(t) \int_{0}^{t} S_{t-t'} (u^{2})_{xx} dt'$.}
...
%
%
Let $f(x,t) \doteq \psi_{T}(t) \int_{0}^{t} S_{t-t'} (u^{2})_{xx} dt'$. 
Then
%
%
\begin{equation}
  \begin{split}
    \wt{f}(n, t)
    & = \psi_{T} \int_{0}^{t}-n^{2}\wt{u^{2}}(n, t') \left[
    \frac{e^{in^{2}(t-t')} - e^{-in^{2}(t-t')}}{2 i n^{2}}
    \right] dt'
    \\
    & = - \frac{1}{2i} e^{in^{2}t} \int_{0}^{t} \psi_{T}(t) \wt{u^{2}}(n, t')
    e^{-in^{2}t'} dt' + 
    \frac{1}{2i} e^{-in^{2}t} \int_{0}^{t} \psi_{T}(t) \wt{u^{2}}(n, t')
    e^{in^{2}t'} dt' \\
...
%
\begin{gather*}
  w_{1}(x,t) = \frac{1}{4 i \pi} \sum_{n \in \zz} e^{inx} \left[ \int_{0}^{t} \psi_{T}(t) \wt{u^{2}}(n, t') e^{in^{2}t'}
  dt'\right],
  \\
  w_{2}(x,t) = \frac{1}{4 i\pi} \sum_{n \in \zz} e^{inx} \left[ \int_{0}^{t} \psi_{T}(t) \wt{u^{2}}(n, t') e^{-in^{2}t'} dt'
 \right].
\end{gather*}
%
%
...
\| f \|_{X_{s,b}}^{2}
  ...
  \|^{2}_{H_{t}^{b}}
  \\
  & = \sum_{n \in \zz} \| \psi_{T}(t) \int_{0}^{t} \wt{u^2}(n, t')
...
Let $-1/2 < b' \le 0 \le b \le b' +1$ and $T \le 1$. Then
%
%
\begin{equation*}
\begin{split}
  \| \psi_{T}(t) \int_{0}^{t} g(t') dt' \|_{H^{b}_{t}} \le c_{\psi}T^{1-(b - b')} \| g
  \|_{H_{t}^{b'}}
\end{split}
\end{equation*}
%
where $c_{\psi}$ is a constant depending only on $\psi$.
...
\begin{equation*}
\begin{split}
  & c_{\psi} T^{1- (a + b)}\left[ \sum_{n \in \zz} (1 + |n|)^{s} \| \wt{u^{2}}(n, t')
  e^{in^{2}t'} \|_{H_{t}^{-a}}  +
...
\begin{equation}
\begin{split}
  \|\psi_{T}(t) \int_{0}^{t} S_{t-t'} (u^{2})_{xx} dt'\|_{X_{s,b}} \le
  c_{\psi} T^{[1- (a + b)]/2}\| u^{2}
...
so \eqref{eqn:non-lin-bound} holds. Furthermore, \autoref{prop:bilin-est} holds,
...
%
%%
Hence, $L=L_{u_0, u_1, \psi, T}$ maps the ball $B\left( 0, \frac{1}{4c} \right) \subset
...
L_{u_0, u_1}u = L_{u_0, u_1, \psi, T}u$, $v = L_{v_0, v_1} v = L_{v_0, v_1, \psi, T}v$, and so
%
%
\begin{equation}
	\label{gen-1a}
	\begin{split}
    & L_{u_0, u_1}(u) - L_{v_0, v_1}(v)
		\\
...
\begin{equation*}
	\begin{split}
    \|u -v \|_{X_{s,b}}
    & = \|L_{u_0, v_0}(u) - L_{u_1, v_1}(v) \|_{X_{s,b}}
    \\
    & \le
...
\begin{equation}
	\begin{split}
		\|u -v \|_{X_{s,b}} \le 2 c_{\psi} \left( \|u_0 -v_0 \|_{H^s\left( \ci \right)} +\|u_1 -v_1
        \|_{H^{s-2}\left( \ci \right)} \right ).
	\end{split}
...
\begin{equation*}
\begin{split}
  \psi(t) \int_{0}^{t} g(t') dt'
  & = \frac{\psi(t)}{2 \pi} \int_{0}^{t} \int_{\rr} e^{it' \tau}
  \wh{g}(\tau) d \tau dt'
  \\
  & \simeq \psi(t) \int_{\rr} \int_{0}^{t} e^{it' \tau} \wh{g}(\tau) dt' d\tau
  \\
  & = \psi(t)  \int_{\rr} \frac{e^{it \tau} -1}{i \tau}
  \wh{g}(\tau) d \tau
  \\
  & = \psi(t) \int_{| \tau |T \le 1} \frac{e^{it\tau}
  -1}{i\tau}\wh{g}(\tau) d \tau + \psi(t) \int_{| \tau |T \ge 1} \frac{e^{it\tau}
  -1}{i\tau}\wh{g}(\tau) d \tau
  \\
  & = \psi(t) \sum_{k \ge 1}\frac{t^{k}}{k!} \int_{| \tau |T \le 1}
  (i\tau)^{k-1} \wh{g}(\tau) d \tau + \psi(t) \int_{| \tau |T \ge 1}
  \frac{e^{it \tau}}{i \tau}\wh{g}(\tau) d \tau
  \\
  & - \psi(t) \int_{| \tau |T \ge 1}\frac{\wh{g}(\tau)}{i \tau} d \tau
  \\
  & = I + II + III.
\end{split}
\end{equation*}
...
\begin{equation}
  \label{h1-norm}
\begin{split}
  & \|I \|_{H^{b}} = \sum_{k \ge 1} \frac{1}{k!} \| t^{k} \psi(t) \|_{H^{b}}
  | \int_{| \tau |T \le 1} (i \tau)^{k-1} \wh{g}(\tau) d \tau |
  \\
  & \le \sum_{k \ge 1} \frac{1}{k!} \|t^{k} \psi(t)\|_{H^{b}} T^{1-k}\int_{| \tau |T \le
  1} | \wh{g}(\tau) | d \tau.
\end{split}
\end{equation}
%
...
%\begin{equation}
	%\begin{split}
%\|t^k \psi(t) \|_{H^b(\rr)} 
%& = \left( \int_{\rr}(1 + | \lambda |)^{2b}| t^{k}\psi(\lambda) |^{2}
%\right)^{1/2}
%\\
%& = \left( \int_{\rr} (1 + | \lambda |)^{2b} | \int_{\rr} |e^{-i\lambda
%t}t^{k} \psi(t) dt|^{2} \right)^{1/2}
%\\
%& = \left( \int_{\rr} (1 + | \lambda |)^{2b} | \int_{| t | \le T} |e^{-i\lambda
%t}t^{k} \psi(t) dt|^{2} \right)^{1/2}
%\\
%& \le T^{k}
%\left( \int_{\rr} (1 + | \lambda |)^{2b}  \int_{| t | \le T} |\psi(t)|^{2} dt \right)^{1/2}
%\\
%& = T^{k} \| \psi \|_{H^{b}}
%\\
%& = c_{\psi} T^{k}
...
%\begin{equation}
  %\label{hu}
%\begin{split}
   %c_{\psi} \sum_{k \ge 1} \frac{1}{k!} T \int_{| \tau |T \le 1} |
  %\wh{g}(\tau)| d \tau
  %& \le  c_{\psi}e \int_{| \tau |T \le 1}| \wh{g}(\tau) | d \tau
  %\\
  %& =  T c_{\psi} e \int_{| \tau |T \le 1}| \wh{g}(\tau) | d \tau.
%\end{split}
%\end{equation}
%
%
...
\begin{equation}
  \label{microl-est}
\begin{split}
  & \int_{| \tau |T \le 1}| \wh{g}(\tau) | d \tau
  \\
  & \le  \| g \|_{H^{b'}} \left( \int_{| \tau | T \le 1}  (1 + | \tau
  |)^{-2b'} d \tau \right)^{1/2} 
  \\
  & = \sqrt{2} \| g \|_{H^{b'}} \left( \int_{0 \le \tau \le
  \frac{1}{T}}(1 + \tau)^{-2b'} d \tau \right)^{1/2}
  \\
  & = \sqrt{\frac{2}{-2b'+1}} \left[ (1 + \tau)^{-2b'+1} \Big |_{0}^{1/T}
  \right]^{1/2}
  \\
  & \simeq \left[ (1 + 1/T)^{-2b'+1} - 1 \right]^{1/2}
  \\
  & \le \left[ (2/T)^{-2b'+1} -1 \right]^{1/2}, \qquad T \le 1
  \\
  & \lesssim_{b'} T^{-1/2 + b'}
\end{split}
\end{equation}
...
\begin{split}
  \| t^{k} \psi \|^{2}_{H^{b}} 
  & = \int_{\rr} (1 + | \tau |)^{2b} | \wh{t^{k}\psi(t)}(\tau) |^{2} d \tau
  \\
  & = \int_{\rr} (1 + | \tau |)^{2b} | \int_{\rr} e^{-it \tau} t^{k}
  \psi(t) dt |^{2} d \tau
  \\
  & = \int_{\rr} (1 + | \tau |)^{2b} | \int_{| t | \le T} e^{-it \tau} t^{k}
  \psi(t) dt |^{2} d \tau.
    \end{split}
\end{equation*}
%
%
Using the change of variable $\lambda = \tau/T$, this is equal to
%
%
%
\begin{equation*}
\begin{split}
  & T \int_{\rr} \left (1 + \frac{| \lambda |}{T} \right )^{2b}
  | \int_{| t | \le T} e^{-itT \lambda}
  t^{k} \psi(t) dt |^{2} d \lambda  \\
  & \le T^{1 - 2b} \int_{\rr} (1 + | \lambda |)^{2b}
  | \int_{| t | \le T} e^{-itT \lambda} t^{k} \psi(t) dt |^{2} d \lambda,  
 \quad T \le 1 \\
  \\
  & \simeq_{b, \psi} T^{2k+1-2b}.
\end{split}
\end{equation*}
%
%
...
\begin{equation}
\begin{split}
  \| t^{k}\psi(t) \|_{H^b} \lesssim_{b, \psi}  T^{k+1/2 -b}
\end{split}
\label{hb-norm}
\end{equation}
%
...
\begin{split}
  \| I \|_{H^{b}}
  & \lesssim_{\psi, b, b'} \sum_{k \ge 1}\frac{1}{k!} T^{k + 1/2 -b}
  T^{1-k} T^{-1/2 + b'}
  \\
  & = \sum_{k \ge 1}\frac{1}{k!}T^{1 - (b - b')}
  \\
  & \le e T^{1 - (b - b')}
  \\
  & \simeq T^{1- (b - b')}.
...
\begin{split}
  \| II \|_{H^{b}}
  & = \| \psi(t) \int_{| \tau |T \ge 1} \frac{\wh{g}(\tau)}{i
  \tau} d \tau \|_{H^{b}}
  \\
  & = | \int_{| \tau | T \ge 1} \frac{\wh{g}(\tau)}{i \tau} d \tau | \times \| \psi
  \|_{H^{b}}
  \\
  & \le \int_{| \tau |T \ge 1} \frac{| \wh{g}(\tau)|}{ |\tau|} d \tau  \times \| \psi
  \|_{H^{b}}.
\end{split}
\end{equation}
%
...
\begin{equation*}
\begin{split}
\int_{| \tau |T \ge 1} \frac{| \wh{g}(\tau)|}{ |\tau|} d \tau 
& = \int_{| \tau |T \ge 1} \frac{| \wh{g}(\tau)| (1 + | \tau |)^{b'} (1 + | \tau
|)^{-b'}}{ |\tau|} d \tau 
\\
& \le \|g\|_{H^{b'}} \left[ \int_{| \tau | T \ge 1} (1 + | \tau |)^{-2b'} | \tau
|^{-2} d \tau \right]^{1/2}.
\end{split}
\end{equation*}
%
%
Applying the inequality $(1 + | \tau |)^{2} \le 4 | \tau |^{2}, \quad | \tau
| \ge 1$, and observing that $| \tau | T \ge 1$ implies $| \tau | \ge 1$ for $T \le
1$, we bound the integral term by
%
%
%
%
\begin{equation*}
\begin{split}
\left[ \int_{| \tau | T \ge 1} 4 (1 + | \tau |)^{-2b'-2} | \tau
|^{-2} d \tau \right]^{1/2}
& = \left[ \int_{1/T \le  \tau < \infty } 8 (1 +  \tau )^{-2b'-2} 
d \tau \right]^{1/2}
\\
& = c_{b'} \left [ \left( 1 + \frac{1}{T} \right)^{-2b' -1} \right ] ^{1/2}, \quad b'> -1/2
\\
& \le c_{b'} \left [ \left (\frac{2}{T} \right )^{-2b' -1} \right ]^{1/2}, \quad T \le 1
\\
& \simeq_{b'} T^{1/2 + b'}
\end{split}
\end{equation*}
...
\begin{split}
  \|\psi \|_{H^{b}}^{2}
  & = \int_{\rr} (1 + | \tau |)^{2b} | \wh{\psi}(\tau) |^{2} d \tau
  \\
  & = \int_{\rr} (1 + | \tau |)^{2b} | \int_{\rr} e^{-it \tau} \psi(t) dt
  |^{2} d \tau
  \\
  & = \int_{\rr} (1 + | \tau |)^{2b} | \int_{| t | \le T} e^{-it \tau} \psi(t) dt
  |^{2} d \tau
\end{split}
\end{equation*}
%
%
which by the change of variable $s = t/T$ is equal to
%
%
\begin{equation*}
\begin{split}
  T^{2} \int_{\rr}  (1 + | \tau |)^{2b} | \int_{| s | \le 1}
  e^{-isT \tau} \psi(sT) ds |^{2} d \tau
  & = T^{2} \int_{\rr} \int_{\rr} (1 + | \tau |)^{2b} | \int_{\rr}
  e^{-isT \tau} \psi(sT) ds |^{2} d \tau
  \\
  & = T^{2} \int_{\rr} (1 + | \tau |)^{2b} | \wh{\psi(sT)}(T \tau) |^{2} d \tau.
\end{split}
\end{equation*}
%
%
Applying the change of variable $\lambda = T \tau$, the above is equal to
%
%
\begin{equation*}
\begin{split}
  & T \int_{\rr} \left( 1 + \frac{| \lambda |}{T} \right)^{2b} |
  \wh{\psi(sT)}(\lambda) |^{2} d \lambda
  \\
  & \le T \int_{\rr} \left( \frac{1}{T} + \frac{| \lambda |}{T}
  \right)^{2b} |
  \wh{\psi(sT)}(\lambda) |^{2} d \lambda, \quad T \le 1
  \\
  & \simeq T^{1 - 2b} \int_{\rr} (1 + | \lambda |)^{2b}
  | \wh{\psi(sT)}(\lambda) |^{2} d \lambda
  \\
  & \simeq_{b, \psi} T^{1-2b}.
\end{split}
...
\begin{equation}
  \label{est-II-2}
\begin{split}
  \| \psi \|_{H^{b}} \lesssim_{b, \psi} T^{1/2 - b}
\end{split}
\end{equation}
%
%
Substituting \eqref{est-II-1} and \eqref{est-II-1} into the right hand side of
\eqref{est-II}, we obtain
%
%
\begin{equation*}
\begin{split}
  \| II \|_{H^{b}} \lesssim_{\psi, b, b'} T^{1 - (b - b')}.
\end{split}
\end{equation*}
%
%
...
%\begin{equation}
%\begin{split}
  %\eqref{hu} & \lesssim_{\psi} T^{1/2 - a/2}
  %\\
  %& \le T^{1 - (a +b)}
%\end{split}
%\end{equation}
