...
%
%
is a solution to \cref{main-rel-term-1}-\cref{main-rel-term-5} for $t \in
[-\delta, \delta]$.  
%
%
%
...
norm of \cref{main-rel-term-1}-\cref{main-rel-term-5}. The 
...
see this, we estimate \cref{main-rel-term-1}. We have
%
%
\begin{equation*}
  \begin{split}
    \wh{\cref{main-rel-term-1}}
...
\begin{lemma}
  ...
  \begin{equation*}
  \begin{split}
    \| \psi(t/\delta) \|_{X_{s,b'}} \lesssim_{\psi, b, b'} \delta^{b - b'} \| u
    \|_{X_{s,b}}.
  \end{split}
  \end{equation*}
  %
...
%
%
Restricting $1/2 < b < 3/4$, we let $b' = -(1-b)$ and apply \cref{lem:time-local-est} to
\eqref{pre-bi-est} to obtain the bound
%
\begin{equation}
  \label{gff}
  \begin{split}
    & 4 \delta^{1-b -a } \left( \sum_{n \in 
		\zz} \left (1 + |n| \right )^{2s} \int_\rr
		\frac{|\wh{w}(n, \tau) |^2}{(1+ |  |\tau| - 
    n^{m}|)^{2a}} 
		 \ d \tau 
...
\end{split}
\end{equation}
where $ -1/2 < a \le 1-b < 1/2$. We now need the following bilinear estimate, whose proof we leave for later.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
\begin{equation}
	\begin{split}
    \|\cref{main1-rel-term-3}\|_{X_{s,b}} \le c_{\psi, b} \delta^{1/2-b}
    \delta^{1-b -a}\|f\|_{X_{s,b}}
    \|g\|_{X_{s,b}}.
	\end{split}
\end{equation}
%
...
\begin{equation*}
    ...
    \delta^{\ee}\|f\|_{X_{s,b}}
    \|g\|_{X_{s,b}}, \quad 0 < \ee < 1/12, \ b = 1/2 + \ee, \ s \ge -1/4 + 3 \ee 
	\end{split}
\end{equation*}
%
%
...
\begin{equation}
	...
	\begin{split}
    \|\cref{main1-rel-term-3}\|_{X_{s,b}} \le c_{\psi, b} 
    \delta^{b-1/2}\|f\|_{X_{s,b}}
    \|g\|_{X_{s,b}}, \quad 1/2 < b <7/12,  \ s \ge -1/4 + 3(b -1/2) 
	\end{split}
\end{equation}
...
\begin{equation}
  \label{iu}
	\begin{split}
		...
		\\
    & \le c_{\psi, b} \delta^{1/2-b} \sum_{a = \pm 1} \left( \sum_{n \in \zz} \left (1 + |n| \right)^{2s}
...
\begin{equation}
	\label{main-int3-est}
	\begin{split}
		\|\cref{main-rel-term-4}\|_{X_{s,b}} 
    \le c_{\psi, b} \delta^{1/2-b} \|f\|_{X_{s,b}} \|g\|_{X_{s,b}}. 
	\end{split}
\end{equation}
%
%
...
\begin{equation*}
\begin{split}
  \wh{t^{k} \psi_{\delta}}(\tau) 
  & = \int_{\rr} e^{-it \tau} t^{k} \psi_{\delta}(t) dt
  \\
  & = \int_{\rr} e^{-it \tau} t^{k} \psi(\frac{2t}{\delta})dt
  \\
  & = \int_{\rr} e^{-i \tau s \delta /2} \left( \frac{s \delta}{2}
  \right)^{k} \psi(s) \frac{\delta}{2} ds
  \\
  & = \left( \frac{\delta}{2} \right)^{k+1} \wh{s^{k} \psi}
  \left(\frac{\delta \tau}{2} \right )
\end{split}
\end{equation*}
%
%
...
\begin{equation*}
\begin{split}
  \int_{\rr} (1 + | \tau' |)^{2b} | \wh{t^{k} \psi_{\delta}}(\tau') |^{2} d
  \tau'
  & = \left( \frac{\delta}{2} \right)^{2k+2} \int_{\rr} (1 + | \tau'
  |)^{2b} \wh{s^{k} \psi}\left( \frac{\delta \tau' }{2} \right)d \tau'
  \\
  & = \left (\frac{\delta}{2} \right )^{2k +1} \int_{\rr} \left( 1 + | \frac{2 \lambda}{\delta} |
  \right)^{2b} \wh{s^{k} \psi}(\lambda) d \lambda
  \\
  & \le \left (\frac{\delta}{2} \right )^{2k+1} \int_{\rr} \left( \frac{1}{\delta} + | \frac{2 \lambda}{\delta} |
  \right)^{2b} \wh{s^{2k} \psi}(\lambda) d \lambda
  \\
  & \simeq \left( \frac{\delta}{2} \right)^{2k +1 -2b} \| s^{k} \psi
  \|_{H^{b}}^{2}
  \\
  & \le \left( \frac{\delta}{2} \right)^{2k +1 -2b} \| s^{k} \psi \|_{H^{[b] +
  1}}^{2}
\end{split}
\end{equation*}
%
...
\label{main-int4-est}
	\begin{split}
    \|\cref{main-rel-term-5}\|_{X_{s,b}} \le c_{\psi} \delta^{1/2}
    \|u\|_{X_{s,b}}^{3}.
	\end{split}
\end{equation}
%
...
\begin{equation}
	\label{gen-2a}
	\begin{split}
    & \| \cref{main-rel-term-1g}\|_{X^{s}}
		\le c \|u_0 -v_0\|_{H^s},
    \\
    & \| \cref{main-rel-term-2g}\|_{X^{s}}
