...
$s\ge 
-1/2$ in the periodic and non-periodic cases.
\end{theorem} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                Proof of Thm
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\label{sec:periodic}
What we are interested in is estimating the $\dot{X}_{s} \doteq X_{s,1/2}$ norm of
...
= \frac{\wh{\psi}(\tau -
\gamma(n)^{2})\wh{u_{0}}(n)}{2} + \frac{\wh{\psi}(\tau +
\gamma(n)^{2})\wh{u_{0}}(n)}{2}.
\end{split}
\end{equation*}
...
gives
\begin{align*}
\| \cref{main1-rel-term-1} \|_{\dot{X}_{s}}^{2} 
& \lesssim \sum_{n \in \zzdot} |n|^{2s} | \wh{u_{0}} |^{2} \int_{\rr}| \wh{\psi}(\tau - \gamma(n)^{2}) +
\wh{\psi}(\tau + \gamma(n)^{2}) |^{2}
...
& \lesssim \| u_{0} \|^{2}.
\end{align*}
where the last step follows once we break the integral into the regions $| \tau | \ge 2\gamma(n)^{2}$, $0 \le \tau \le 2\gamma(n)^{2}$, and $-2\gamma(n) \le \tau \le 0$, apply the triangle inequality, and estimate separately. 
Taking square roots, we obtain
\begin{equation*}
\begin{split}
\| \cref{main1-rel-term-1} \|_{\dot{X}_{s}} \lesssim \| u_{0} \|_{\dot{H}^{s}}.
\end{split}
\end{equation*}
...
\begin{equation*}
\begin{split}
\| \cref{main1-rel-term-2} \|_{\dot{X}_{s}}^{2} 
&  
\lesssim \sum_{n \in \zzdot} |n|^{2s-4} | \wh{u_{0}} |^{2} \int_{\rr}| \wh{\psi}(\tau - \gamma(n)^{2}) +
\wh{\psi}(\tau + \gamma(n)^{2}) |^{2}
\frac{\langle \tau - \gamma(n)^{2} \rangle  \langle \tau + \gamma(n)^{2} \rangle }{\langle \tau - \gamma(n)^{2} \rangle  + \langle \tau + \gamma(n)^{2} \rangle }
d \tau
\notag
\\
& \lesssim \| u_{1} \|_{\dot{H}^{s-2}}^{2} 
\\
& = \| \vp_{x} \|_{\dot{H}^{s-1}}.
\end{split}
\end{equation*}
...
\label{u-1-est}
\begin{split}
\|\cref{main1-rel-term-2}|_{X_{s}} \lesssim 
\|\vp_{x}\|_{\dot{H}^{s-1}}.
\end{split}
\end{equation}
...
\label{schwartz-mult}
\begin{split}
\|\psi f \|_{\dot{X}_{s}} \lesssim_{\psi} \|f \|_{\dot{X}_{s}}.
\end{split}
\end{equation}
\end{lemma}
Hence
...
\label{yu}
\begin{split}
\|\cref{main1-rel-term-3}\|_{\dot{X}_{s}}^{2} 
& \lesssim_{\psi} 
\sum_{a = \pm 1} \| \sum_{n \in \zzdot}  e^{ixn} \int_\rr 
e^{it \tau} \frac{1 - a\psi (\tau - a\gamma(n)^{2} ) 
}{\tau - a\gamma(n)^{2}} \wh{w}(n, \tau) \ 
d \tau\|_{\dot{X}_{s}}^{2}
\\
& \lesssim \sum_{a = \pm 1}\sum_{n \in \zzdot} |n|^{2s} \int_\rr
\frac{\langle \tau - \gamma(n)^{2} \rangle  \langle \tau + \gamma(n)^{2} \rangle }{\langle \tau - \gamma(n)^{2} \rangle  + \langle \tau + \gamma(n)^{2} \rangle } 
\left | \frac{1 - a\psi(\tau - an^{2 
})}{\tau - a\gamma(n)^{2}} 
\wh{w}(n, \tau) \right |^2 \ d 
\tau 
\\
& \le \sum_{a = \pm 1}
\sum_{n \in \zzdot}  |n|^{2s} \int_{| \tau - a\gamma(n)^{2}| \ge 1}
\frac{\langle \tau - \gamma(n)^{2} \rangle  \langle \tau + \gamma(n)^{2} \rangle }{\langle \tau - \gamma(n)^{2} \rangle  + \langle \tau + \gamma(n)^{2} \rangle } 
\frac{|  \wh{w}(n, \tau)|^2}{|\tau - a\gamma(n)^{2}|^2} 
\ d 
\tau 
\\
& \lesssim \sum_{a = \pm 1} \sum_{n \in 
\zzdot} |n|^{2s} \int_\rr \frac{\langle \tau + a\gamma(n)^{2} \rangle }{\langle \tau - a \gamma(n)^{2} \rangle [\langle \tau - \gamma(n)^{2} \rangle  + \langle \tau + \gamma(n)^{2} \rangle ] }
| \wh{w}(n, \tau) |^2 
\ d \tau 
\\
& \lesssim \| u \|^{2}_{\dot{X}_{s}}
\end{split}
\end{equation}
where the last step follows from the following two bilinear
estimates, whose proofs we leave for later.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%				Proposition
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{proposition}
...
\begin{equation}
\left ( \sum_{n \in 
\zzdot} |n|^{2s} \int_\rr \frac{\langle \tau - n^{2} \rangle }{\langle \tau + n^{2} \rangle [\langle \tau - n^{2} \rangle  + \langle \tau + n^{2} \rangle ] }
| \wh{w}(n, \tau) |^2 \right )^{1/2} \lesssim \| f \|_{\dot{X}_{s}}\| g \|_{\dot{X}_{s}}
\end{equation}
\end{proposition}
\begin{proposition}
\label{prop:bilinear-est-2}
If $s \ge -1/2$ then 
\begin{equation}
\left ( \sum_{n \in 
\zzdot} |n|^{2s} \int_\rr \frac{\langle \tau + n^{2} \rangle }{\langle \tau - n^{2} \rangle [\langle \tau - n^{2} \rangle  + \langle \tau + n^{2} \rangle ] }
...
\end{proposition}
\begin{framed}
In fact, as we shall see, the proof of \cref{prop:bilinear-est-2} reduces to that of \cref{prop:bilinear-est} via an appropriate change of variables. 
\end{framed}
Taking square roots, we obtain
\begin{equation}
\begin{split}
\|\cref{main1-rel-term-3}\|_{\dot{X}_{s}} \lesssim
\|u\|^{2}_{\dot{X}_{s}}.
\end{split}
\end{equation}
...
\begin{equation*}
\begin{split}
& g_k(x,t) = t^k \psi(t) \sum_{n \in \zzdot}  e^{i\left( xn + ta \gamma(n)^{2}
\right)} h_k(n),
\\
& h_k(n) = \int_\rr \psi \left( \tau - a\gamma(n)^{2} \right) \cdot \left(
\tau - a\gamma(n)^{2} \right)^{k -1} \wh{w}(n, \tau) \ d \tau.
\end{split}
\end{equation*}
Hence
\begin{equation*}
\begin{split}
\wh{g_k^x}(n, t) = t^{k} \psi(t) e^{i t a\gamma(n)^{2}} h_k(n)
\end{split}
\end{equation*}
which gives
\begin{equation*}
\begin{split}
\wh{g_k}(n, \tau)
& = h_k(n) \int_\rr e^{-it\left( \tau - a\gamma(n)^{2} \right)}
t^{k}\psi(t) \ dt
\\
& = h_k(n) \wh{t^{k}\psi(t)} \left( \tau - a\gamma(n)^{2} \right).
\end{split}
\end{equation*}
...
\begin{split}
& \sum_{a = \pm 1}
\sum_{k \ge 1} \frac{2^{k}}{k!}
\times  \sum_{n \in \zzdot} (1 + | n |)^{2s}| 
\int_{| \tau - a\gamma(n)^{2}  |\le 1}  \wh{w}(n, \tau) \ d \tau |^2 
\\
& \lesssim \sum_{a = \pm 1} \sum_{n \in \zzdot} (1 + | n |)^{2s}
\int_\rr \frac{|\wh{w}(n, \tau)|}{1 + | \tau - a\gamma(n)^{2} |} \ d \tau 
\\ 
& \lesssim \| u \|_{\dot{X}_{s}}^{2}
\end{split}
\end{equation*}
where the last step follows from \cref{prop:bilin-1-endpoint} and \cref{prop:bilin-2-endpoint}.
Hence, taking square roots, we obtain 
\begin{equation*}
\begin{split}
\| \cref{main1-rel-term-4.5} \|_{\dot{X}_{s}} \lesssim \| u \|_{\dot{X}_{s}}^{2}.
\end{split}
\end{equation*}
Collecting our estimates for  
\cref{main1-rel-term-1}-\cref{main1-rel-term-4.5}, we obtain
the following.
\begin{proposition}
...
\begin{equation*}
\begin{split}
\|Tu\|_{\dot{X}_{s}} \lesssim_{\psi} \left( \|u_0 \|_{\dot{H}^s(\ci)} + \|u_1 \|_{\dot{H}^{s-2}(\ci)}
+ \|u\|_{\dot{X}_{s}}^2 
\right).
\end{split}
\end{equation*}
...
\\
& = \|u\|_{\dot{X}_{s}} \|v\|_{\dot{X}_{s}} \label{holder-term}
...
\begin{split}
& \|u\|_{\dot{X}_{s}} \|v\|_{\dot{X}_{s}} \| \phi \|_{L^{2}_{n, \tau}}
\|\bigg (  \langle n
