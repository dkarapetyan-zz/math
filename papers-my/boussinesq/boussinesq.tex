...
\end{lemma}
%
{\bf Proof.} Substituting the ansatz \cref{2nd-order-ansatz} into
...
norm of \cref{main1-rel-term-1}-\cref{main1-rel-term-5}. The 
...
$u \in C([-\delta, \delta], H^s(\ci)) \cap X_{s,b}$ solving $\psi_{\delta} u = Tu$ for $| t | \le \delta$.
...
\begin{equation*}
    ...
    \times \left( \sum_{n \in \zz} (1 + | n |)^{2s}| 
		...
		\right)^{1/2}
    \\
    & = ec_{\psi, b} \sum_{a = \pm 1} \left( \sum_{n \in \zz} (1 + | n |)^{2s}| 
		\int_{| \tau - an^{m}  |\le 1}  \wh{w}(n, \tau) \ d \tau |^2 
		\right)^{1/2}
    \\
    & \le e c_{\psi, b}  \sum_{a = \pm 1}
...
\label{main-int4-est-X-s-part}
	\begin{split}
    & 4e c_{\psi, b} \left[ \sum_{n \in \zz} (1 + | n |)^{2s}\left
    ( \int_\rr
...
\cref{integral-bound-1st-form-per*},
...
%
%
\section{Ill-Posedness for $s < -3/2$ on the Circle}
...
\begin{proposition}
  ...
  \begin{split}
    ...
    \\
    & A_{3}(f,g) \doteq N(A_{1}(f,g), A_{2}(f,g)).
  \end{split}
  \end{equation*}
  %
  %
...
\begin{split}
 ...
 & \ge
  ...
  \wh{L(f_{N},g_{N})}(n - n_{1}, t')\wh{L(f_{N},g_{N})}(n_{1}, t') dt' \|_{\ell^{2}_{n}}
  \\
  & = \sup_{0 \le t \le \delta} \| (1 + | n |)^{s} \frac{1}{4 i \pi} 
  \int_{0}^{t} \sum_{n_{1}} \left( e^{in^{2}(t-t')} - e^{-in^{2}(t-t')} \right)
  \psi(t')
...
\end{equation*}
%
Take $\wh{g_{N}}(k) \equiv 0$. Then the above reduces to
%
%
\begin{equation*}
\begin{split}
...
\|_{\ell_{2}}
\\
& \gtrsim_{s} \sup_{0 \le t \le \delta} 
...
\end{equation}
%
Assume $s <0$, and let $t \doteq N^{-\alpha}$, where we restrict $N \le
\delta^{1/\alpha}$. Then \cref{rxx} is bounded below by
%
%
\begin{equation*}
\begin{split}
  r^{2}N^{-2s -2 \ee}  
  \int_{0}^{N^{-\alpha}} \sin[(N^{-\alpha}-t')] [\cos(N^{2 -\alpha})]^{2} dt'
\end{split}
\end{equation*}
which for $\alpha \ge 2$ is bounded below by
%
%
\begin{equation*}
\begin{split}
...
%
%
By l'H\^opital's rule, for large $N$ we have
%
%
\begin{equation*}
\begin{split}
\frac{1}{4} r^{2} N^{-2s -2 \ee}  
[1 - \cos(N^{-\alpha})]
& \simeq \frac{[1 - \cos(N^{-\alpha})]}{1/N^{-2s -2\ee}}
\\
& \gtrsim \frac{\sin(N^{-\alpha}) N^{-\alpha}}{1/N^{-2s -2\ee + 1}}
\\
& = \frac{\sin(N^{-\alpha})}{N^{2s + 2 \ee -1 + \alpha}}.
\end{split}
\end{equation*}
%
%
...
\begin{equation*}
\begin{split}
  \lim_{N \to \infty} \frac{\sin(N^{-\alpha})}{N^{2s + 2 \ee -1 + \alpha}}
  & = \lim_{N \to 0}\frac{\sin(N^{\alpha})}{N^{-2s - 2 \ee +1 - \alpha}}
  \\
  & = 1, \quad \alpha = -2s -2 \ee +1 - \alpha
\end{split}
\end{equation*}
%
%
or $\alpha = -s -\ee + 1/2$. Since $\alpha \ge 2$, we obtain
%
%
$-s - \ee + 1/2 \ge 2$, or $s \le -3/2 - \ee$, which completes the proof.
\qquad \qedsymbol
%
%
\newpage
...
%
%
where $c = c_{\psi, s, \delta}$ for $-1/4 < s < 0$ and $c =c_{\psi}$ for $s \ge
0$.  
%
%
\begin{framed}
