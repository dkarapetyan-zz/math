...
\begin{definition}
  ...
  %
  \begin{gather*}
    \|F\|_{Z_{s}} = \|P_{| \tau| \ge 2n^{2}}F\|_{X_{s,1/2}} + \min \left \{ \|
    P_{0 \le \tau \le 2n^{2}} u  \|_{X_{s+2,0}}, \| P_{0 \le \tau \le 2n^{2}} u
    \|_{X_{s,1/2}}  \right \} 
    \\
    + \min \left \{ \| P_{-2n^{2} \le \tau \le 0} u
    \|_{X_{s+2,0}}, \| P_{-2n^2 \le \tau \le 0} u  \|_{X_{s,1/2}}  \right \}
  \end{gather*}
\end{definition}
%
...
\end{equation*}
%
it follows that $\left \{ P_{0 \le \tau \le 2n^{2}} u_{n} \right \}$ is Cauchy in $X_{s+2,0}$ and $X_{s,1/2}$. Similarly, $\left \{P_{-2n^{2} \le \tau \le 0} u_{n} \right \}$ is Cauchy in $\mathcal{X}_{s+2,0}$ and $\mathcal{X}_{s,1/2}$.
...
\begin{equation*}
  \begin{split}
    ...
    \\
    & \lesssim \sum_{n \in \dot{\zz}} n^{-4} \left( 1 + |n| \right)^{2s} | \wh{u_{1}}(n)
    |^{2} \int_{\rr}  (1 + | \tau - n^{2} |)| \wh{\psi}(\tau - n^{2}) |^{2} \right) d \tau
    \\
    & + |\wh{u_{1}}(0)|^{2} \int_{\rr} (1 + | \tau |) |\frac{d}{d \tau}
    \wh{\psi}(\tau)|^2 d \tau
    \\
...
\begin{equation}
  \label{u-1-est}
  \begin{split}
    \|\cref{main1-rel-term-2}|_{Z_{s}} \le c_{\psi} 
    \|u_{1}\|_{H^{s-2}}.
  \end{split}
\end{equation}
%
...
\begin{equation}
	\begin{split}
		\|P_{0 \le  \tau  < 2n^{2}}\cref{main1-rel-term-3}\|_{X_{s-1,0}}^{2} 
    & \lesssim
    \sum_{a = \pm 1} \| P_{| \tau | < 2n^{2}} \sum_{n \in \zz} e^{ixn} \int_\rr 
		e^{it \tau} \frac{1 - a\psi (\tau - an^{2} ) 
}{\tau - an^{2}} \wh{w}(n, \tau) \ 
		d \tau\|_{X_{s-1,0}}^{2}
		\\
    & = \sum_{a = \pm 1}\sum_{n \in \zzdot} \chi_{| \tau | < 2n^{2}}\left (1 + |n| \right )^{2(s-1)} \int_\rr
    \left | \frac{1 - a\psi(\tau - an^{2 
     ...
     \wh{w}(n, \tau) \right |^2 \ d 
		...
		\\
    & \le \sum_{a = \pm 1}
    \sum_{n \in \zzdot} \chi_{| \tau | < 2n^{2}} \left (1 + |n| \right )^{2(s-1)} \int_{| \tau - an^{2}| \ge 1}
    \frac{|  \wh{w}(n, \tau)|^2}{|\tau - an^{2}|^2} 
		\ d 
		\tau 
		\\
		& \lesssim \sum_{a = \pm 1}
 \sum_{n \in 
		\zzdot} (1-\chi_{| \tau | \ge 2n^{2}}) \left (1 + |n| \right )^{2(s-1)} \int_\rr (1 +  | \tau - n^{2}|)^{-2} | \wh{w}(n, \tau) |^2 
		 \ d \tau 
		\\
		& \lesssim \| P_{| \tau | \ge 2n^{2}}u \|^{2}_{X_{s,1/2}}
\end{split}
...
\label{prop:bilinear-est2}
	...
	If $s > -1$ then 
	\begin{equation}
	  \| P_{| \tau | < 2n^{2}}w_{fg} \|_{\dot{X}_{s,-1}}
		    \lesssim \|P_{| \tau | < 2n^{2}}f\|_{X_{s,-1}} \|P_{| \tau | \ge 2n^{2}}g\|_{X_{s,-1}}
	\end{equation}
...
\begin{equation}
  \label{iu}
	\begin{split}
		& \| \cref{main1-rel-term-4} \|_{X_{s,1/2}} 
		\\
    & \lesssim 
    \sum_{a = \pm 1} \left( \sum_{n \in \zzdot} \left (1 + |n| \right)^{2s}
    \int_\rr \left( 1 + | \tau - n^{2} \right ) | | \wh{\psi}\left(
    \tau - an^{2} \right) |^2 \ d \tau \right.
		\\
		& \times \left . |
		\int_\rr \frac{1 - \psi\left( \lambda - an^{2} \right)}{\lambda -
		an^{2}} \wh{w}(n, \lambda) \ d \lambda |^2  \right)^{1/2}.
  \end{split}
\end{equation}
...
\int_\rr \left( 1 + | \tau - n^{2} \right ) | | \wh{\psi}\left(
    ...
    \\
    & \le  \sum_{a = \pm 1} 
    \int_\rr \left( 1 + |\tau - an^{2} | \right ) | | \wh{\psi}\left(
    \tau - an^{2} \right) |^2 \ d \tau 
    \\
    & = c_{\psi}.
\end{split}
...
\begin{equation}
  \begin{split}
    ...
    & \le \sum_{a = \pm 1} \left( \sum_{n \in \zzdot} \left (1 + |n| \right )^{2s}  \left ( \int_{| \lambda - 
		an^{2} | \ge 1}
		\frac{|\wh{w}(n, \lambda) |}{|\lambda - an^{2}|}
...
\begin{equation}
	...
	\begin{split}
	  &  \sup_{\|a_{n}\|_{\dot{\ell}^{2}}=1}| \sum_{n \in \zzdot} \left (1 + |n| \right )^{s} a_n
		\int_{\rr}\frac{| \wh{w}(n, \lambda) |}{1 + | |\lambda| - n^{2} |} \ d \lambda |
		\\
		& \le \sup_{\|a_{n}\|_{\dot{\ell}^{2}}=1} \left( \sum_{n \in \zzdot} | a_{n} |^2\int_{\rr} \frac{1}{\left( 1 + | |\lambda| - n^{2} | \right)^{1 + \eta}} \ d \lambda  
		\right)^{1/2} 
		\left ( \sum_{n \in \zzdot}\int_{\rr} \left (1 + |n| \right )^{2s} (1 + | | \lambda | - n^{2} |)^{\eta -1} \wh{w}(n, \lambda) 
		|^2\ d \lambda 
		\right)^{1/2}
		\\
...
\begin{equation}
  \label{main-int3-est}
	\begin{split}
    \|\cref{main1-rel-term-3}\|_{X_{s,1/2}} \le c_{\psi} 
     \|u\|^{2}_{X_{s,1/2}}, \quad   s > -1/2.
	\end{split}
\end{equation}
...
\begin{equation}
	\label{2n}
	\begin{split}
		& \|\cref{main1-rel-term-5}\|_{X_{s,1/2}} 
    \\
    & \lesssim \sum_{a = \pm 1} \left( \sum_{n \in \zzdot} \left (1 + |n| \right )^{2s}
    \int_\rr \left( 1 + | \tau - an^{2} | \right)^{2}
...
\label{main-int4-est}
	\begin{split}
    \|\cref{main1-rel-term-5}\|_{X_{s,1/2}} \le c_{\psi} 
    \|u\|_{X_{s,1/2}}^{3}.
	\end{split}
\end{equation}
%
...
\begin{equation*}
	\begin{split}
    \|Tu\|_{X_{s,1/2}} \le c \left( \|u_0 \|_{H^s(\ci)} + \|u_1 \|_{H^{s-2}(\ci)}
    + \|u\|_{X_{s,1/2}}^2 
		\right).
	\end{split}
\end{equation*}
...
By duality and the equivalence of the norms $\| \cdot \|_{X_{s,1/2}}$ and $\| \cdot \|_{\mathcal{X}_{s}}$, it suffices to show that for $s > -a$, 
%
%%
\begin{equation}
	\label{duality-est}
...
\end{split}
  ...
  \begin{equation*}
  \begin{split}
    \sum_{n \in \zzdot} \frac{\langle n_{1}\rangle ^{\gamma(s)-4a  + 2 \eta +\ee}}{\langle n \rangle^{2a - \eta + \ee}} < \infty, \quad s \ge -a + \ee/4 + \eta/2, \ a > 1/2 - \ee/2 + \eta/2
  \end{split}
  \end{equation*}
  %
...
\sum_{n \in \zzdot} \frac{\chi_{B_{3,1}}\langle n_{1} \rangle ^{\gamma(s)-2a + \eta - \ee}}{\langle
    n\rangle ^{2a-\eta  + \ee}} < \infty, \quad s \ge -a + \ee/4 + \eta/2, \ a > 1/2 - \ee/2 + \eta/2.
\end{split}
\end{equation*}
%
%
