...
$\eqref{mNLS-integral-form}$ by $\psi_{\delta}(t)$, we obtain
%
%
\begin{equation}
	\begin{split}
...
\begin{equation}
	\label{main-int2-est}
	\begin{split}
		\|\eqref{main-int-expression-2}\|_{Y^s} \le c_{\psi, \delta}\|f\|_{X^s} \|g\|_{X^s}\|h\|_{X^s}.
	\end{split}
\end{equation}
%
%
...
\label{main-int4-est}
	\begin{split}
    \|\eqref{main-int-expression-4}\|_{Y^s} \le c_{\psi, \delta} \|u\|_{X^s}^{3}.
	\end{split}
\end{equation}
%
%
...
\autoref{lem:cutoff-loc-soln} then yields a unique solution $u \in C(\left[ -\delta,
