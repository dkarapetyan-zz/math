...
= [-1, 1 ]$. Define $\psi_{T}(t) = \psi(2t/T)$.  Multiplying both sides of expression
...
$\psi_{T} u = Tu$. Lipschitz continuity of the flow map (and hence, uniform
...
T \right], H^{s}(\ci))$ to the integral equation $\psi_{T} u = Tu$.\\
\\
...
\begin{equation*}
	\begin{split}
    \|\psi_{T} u - \psi_{T} v \|_{Y^s} = \|T_{\vp_1}(u) - T_{\vp_2}(v) \|_{Y^s} \le c_{\psi,T}
...
\begin{equation*}
	\begin{split}
    \frac{3}{4} \|\psi_{T} u- \psi_{T} v\|_{Y^s} \le c_{\psi,T} \|\vp_1 - \vp_2 \|_{H^s(\ci)}
	\end{split}
\end{equation*}
%
%
...
\begin{equation*}
	\begin{split}
    \|\psi_{T} u - \psi_{T} v \|_{Y^s} \le \frac{4}{3} c_{\psi,T} \|\vp_1 - \vp_2 \|_{H^{s}(\ci)}.
	\end{split}
\end{equation*}
%
%
