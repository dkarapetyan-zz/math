...
\begin{theorem}
	...
	\begin{enumerate}[(i)]
		\item\label{itm:first} $C(K) = C(K, T, S(t), t)$ is a strictly decreasing, strictly convex, Lipschitz continuous
		...
		\item\label{itm:fifth}
			...
			strictly concave, Lipschitz continuous function of $S$. Furthermore,
			\begin{gather*}
					\frac{\p C}{\p S}(S) < 0 \ \text{if}
					\ \frac{\p C}{\p S}(S) \ \text{exists,}
					\\
\frac{\p^{2} C}{\p S^{2}}(S) < 0 \ \text{if}
					\ \frac{\p^{2} C}{\p S^2}(S) \ \text{exists.}
			\end{gather*}
	\end{enumerate}
\end{theorem} 
...
\begin{proof}[Proof of \cref{itm:first}]
	...
	To establish the convexity, observe that applying the identity ${(A + B)}_{+} \le A_{+} + B_{+}$
	gives
	\begin{equation*}
		\begin{split}
...
%\begin{remark}[The Federal Reserve]
	%Concavity in $S$ tells us that, for fixed $t$,
	...
	%\begin{equation*}
		...
		%\end{split}
	%\end{equation*}
	%Hence, for fixed $K$, for $S \gg 1$, the lower bound for $Z_{t}$
...
\end{split}
\end{equation*}
%For the sufficiency, assume there exist $s, t$ such that
%\begin{equation*}
%\begin{split}
...
%\begin{definition}
	%We say that a sequence $F_{1}, \ldots, F_{n}: \rr \to \rr$
	%of distribution functions
	%converge to $F: \rr \to \rr $ if they converge pointwise to $F$. 
%\end{definition}
...
\begin{equation*}
\begin{split}
 \sigma	\sqrt{\Delta t } \sum_{j = 1}^{n} Y_{j} = 
...
\end{split}
\end{equation}
%More generally, for a multi-step model, we have
%\begin{equation}\label{step-prob-gen}
%\begin{split}
	%p_{j \Delta t} = \frac{e^{r j\Delta t} S_{0}(j \Delta t) - S_{-}(j \Delta
%t)}{S_{+}(j \Delta t)
	%- S_{-}(j\Delta t)}.
%\end{split}
%\end{equation}
%where we remark that $S_{0}(j \Delta t)$ is a random variable: there are
%$2^{j} -1$ possible nodes $S_{0}$ after $j$ timesteps of size $\Delta t$. 
...
\bibliography{/Users/davidkarapetyan/Documents/math/bib/references.bib}
