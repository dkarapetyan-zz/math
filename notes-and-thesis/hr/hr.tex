...
Recently  in \cite{Himonas:2010p1187} non-uniform 
...
Himonas, Kenig, and Misiolek \cite{Himonas:2010p1187}.
...
\cite{Tao:2004ck} obtained Lipschitz continuity of the data to solution map
...
\begin{equation}
	...
	\end{split}
\end{equation}
Substituting in \eqref{apple24}, and observing that $\|u^h\|_{H^1(\rr)} \le 1$ for $\lambda >>1$, $ \ge 1$ by 
...
%
Here we follow the proof in \cite{Himonas:2010p1187}. 
...
used for the CH equation in \cite{Himonas:2010p1187}.
%
%
...
taken from \cite{Himonas:2010p1187}:
%
\begin{lemma}
\label{cor1}
...
bounds for the whole range $1 \le r \le s$, where again $s > 3/2$. Applying the
...
s$. We need the following result taken from Taylor \cite{Taylor:1995kx}.
...
theorem, taken from Dieudonn\`e \cite{Dieudonne_1969_Foundations-of-}: 
%
\begin{theorem}
\label{ode_theorem}
