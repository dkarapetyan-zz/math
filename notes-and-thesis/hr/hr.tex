...
%
Notice that
$F$ maps open sets in $C^{k}$ to $C^{k}$. Therefore, the BBM is an ODE on the
space of $C^{k}$ functions. By the ODE theory presented in the appendix, for
$u_{0}(x) \in C^{k}$, the BBM admits a unique solution $u(x,t)$ which is 
$C^{k}$ in both the space and time variables. 
We will show that if $u_{0} \in C^{k}, 1 \le k \le \infty$, then the
data-to-solution map is also $C^{k}$. By the ODE theory and differential
calculus developed in the appendix, it is enough to show that $F$ is $k$ times
differentiable. In fact, we will show that $F$ is smooth. To do this, we adopt
the notation
\begin{gather*}
    F(u) = -\frac{3}{2}Lf(u), \ \text{where} \ 
    \\
    L \doteq \p_{x}(1 - \p_{x}^{2})^{-1}, \ f(u) = u^{2}.
\end{gather*}
Note that $L$ is a linear operator on $C^{k}$, and hence $DL = L$. Applying the chain rule and product rule, we get
%
%
\begin{equation*}
\begin{split}
DF(u) & = -\frac{3}{2}DL(f(u))Df(u)
\\
& = -\frac{3}{2}Lf(u)2u
\\
& = 2u F(u).
...
\begin{split}
    ...
    & = 2 D^{k}(uF(u))
    \\
    & = 2 \sum_{j = 0}^{k} {k \choose j} D^{j}u D^{k - j}F(u)
    \\
...
%
%
We conclude that $F$ is smooth. Therefore, the data-to-solution map for the BBM
equation is $C^{k}$. In particular, due to Sobolev Imbedding, this implies that
the method of proof developed to prove non-uniform dependence for HR does not
apply to the BBM.
