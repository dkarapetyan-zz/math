...
\begin{definition}
	We say that the mNLS ivp \eqref{mNLS-eq}-\eqref{mNLS-init-data} is
	...
	\\
	$u \in C([-T, T],
	H^s(\ci))$ satisfying \eqref{mNLS-eq} for all $t \in [-T, T]$. 
...
\end{equation}
Applying 
the Fourier transform to the mNLS ivp in the space variable we obtain 
%
%
\begin{gather*}
...
%
%
Then it is immediate that \eqref{mNLS-integral-form} is a weaker 
...
$\eqref{mNLS-integral-form}$ by $\psi(t)$, we obtain
%
%
\begin{equation}
	\begin{split}
...
solution $u \in C([-T, T], H^s(\ci))$ to the mNLS ivp which coincides with the solution to
...
\begin{equation}
	\label{main-int2-est}
	\begin{split}
		\|\eqref{main-int-expression-2}\|_{Y^s} \le c_{\psi}\|f\|_{X^s} \|g\|_{X^s}\|h\|_{X^s}.
	\end{split}
\end{equation}
%
%
\subsection{Estimate for \eqref{main-int-expression-3}.}
Letting $$f(x,t) = \psi(t) \sum_{n \in \zz} e^{i\left( xn + tn^{m} \right)} 
...
\begin{equation*}
	\begin{split}
		& g_k(x,t) = t^k \psi(t) \sum_{n \in \zz} e^{i\left( xn + tn^{m}
		\right)} h_k(n),
		\\
...
\label{main-int4-est-X-s-part}
	\begin{split}
		\|\eqref{main-int-expression-4}\|_{X^s}
		& \lesssim \left( \sum_{n \in \zz} | \int_{| \tau - n^{m}| \le 1 }
...
\label{main-int4-est}
	\begin{split}
		\|\eqref{main-int-expression-4}\|_{Y^s} \lesssim \|u\|_{X^s}^{3}.
	\end{split}
\end{equation}
%
%
...
mNLS ivp. Let $c = c_{\psi}^{1/2}$. For given $\vp$, we may choose $\psi$ such
that 
%
%%
\begin{equation*}
...
solution to the mNLS ivp \eqref{mNLS-eq}-\eqref{mNLS-init-data}.
\\
\\
...
%
	 ...
	 %
	 %
Hence, the flow map of the mNLS ivp is locally Lipschitz continuous in
$H^s(\ci)$. This
...
\begin{equation}
	...
	\begin{split}
		 & \left (1 + |n| \right )^s \left( 1 + | \tau - n^{m } | \right)^{-b/2} | \wh{w_{fgh}}\left( 
		n, \tau \right) |
		\\
		& \le \left( 1 + | \tau - n^{m } | \right)^{-b/2}
		\sum_{n_1, n_2, n_3} \int \frac{\left (1 + |n| \right )^s}{\left (1 +
...
\begin{equation*}
	\begin{split}
		\left (1 + |n| \right )^s \left( 1 +  | \tau - n^{m }  | \right)^{-b/2} | 
...
\begin{equation}
	...
	\begin{split}
		& \| \left( 1 + |n | \right)^s
		\left( 1 + | \tau - n^{m } | \right)^{-b/2} \wh{w_{fgh}}(n, 
		\tau)		
		\|_{L^2(\zz \times \rr)}
		\\
		& \lesssim \| \left( 1 + | \tau - n^{m } | \right)^{-b/2}
...
\begin{equation}
	...
	\begin{split}
		\|\left( 1 + | \tau - n^{m } | \right)^{-b/2} \wh{C_f C_{ 
...
\begin{equation*}
	\begin{split}
		\| \left( 1 + |n | \right)^s \left( 1 + | \tau - n^{m} | \right)^{-b/2} \wh{w_{fgh}} 
...
\begin{equation}
	\begin{split}
		| \wh{w_{fgh}}(n, \tau)|
		& = | \sum_{n_1, n_2, n_3}  \int n \wh{f}\left( n_1,  \tau_1 
\right) \wh{g}\left( n_2, \tau_2  
\right) \wh{h}\left( n_3, \tau_3 \right) d \tau_1 d \tau_2 d \tau_3 |
\\
& \le \sum_{n_1,n_2,n_3}  \int | n | \times | \wh{f}\left( n_1, \tau_1 
\right) | \times  | \wh{g}\left( n_2, \tau_2 
\right) | \times | \wh{ h}\left( n_3, \tau_3 \right) | d \tau_1 d \tau_2 d 
\tau_3
\\
& = \sum_{n_1,n_2,n_3} \int \frac{| n |c_f\left( n_1, \tau_1 
\right)}{\left (1 + |n_1| \right )^s \left( 1 + | \tau_1 - n_1^{m} | \right)^{b/2}}
\\
& \times \frac{c_{g}\left( n_2, \tau_2 \right)}{\left (1 + |n_2| \right ) 
^s\left( 1 + | \tau_2 -  n_2^{m }| 
...
unlike the mNLS, we hope that the  principal symbol $\tau - n^m$ offers enough
...
take $n_1 = n_2$.) In the case of the KDV, to prove well-posedness we need to bound
\begin{equation}
...
a lower bound of $|n|^{2}$. By the pidgeonhole principle, we then have three
cases, each with the lower bound
%
%
\begin{equation*}
	\begin{split}
		\frac{1}{| \tau_{i} - n_{i}^{m} |^{b/2}} \gtrsim \frac{1}{|n|^{b}}	
	\end{split}
\end{equation*}
%
%
where $\tau_0 =\tau, n_0 = n$. 
Hence, we must set $b \ge 1$ to offset the $|n|$ in the numerator of the
...
\begin{equation*}
	...
	\end{split}
\end{equation*}
which is already bounded by $3^s$ (note the absense of a $|n|$ term in the
numerator). Hence, we have more ``wiggle room'' with how
we choose $b$. In fact, we can choose $b$ all the way down to $3/4$, but no
lower, since we must have $b/2 \ge 3/8$ in order to be able to apply
...
\begin{lemma}
	...
	\begin{equation*}
		\begin{split}
			|n_{1}^{m}-n^{m} + (n-n_{1})^{m}| \ge | n_{1} |^{m-1}
		\end{split}
	\end{equation*}
	%
	%
...
%
%
{\bf Proof.} Define $f(n) = n_{1}^{m} - n^{m} + (n-n_{1})^{m}$. Then the minimum
of $f(n)$ occurs closest to the lattice point $n = 1+n_{1}$ (the lattice point
$n = n_1$ is not available by assumption). But here
%
%
\begin{equation*}
	\begin{split}
		...
		& = | (1 + n_{1} )^{m} - n_{1}^{m} -1 |
		\\
		& \ge \frac{1}{2} | (1 + n_{1})^{m}  - n_{1}^{m}|
		\\
		& \ge \frac{1}{2}| n_{1} |^{m-1},
	\end{split}
\end{equation*}
